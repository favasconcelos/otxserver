...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
  ...
  elseif target.itemid == 8759 then
    ...
    nexusMessage(
      player,
      player:getName() .. " destroyed the shadow nexus! In 20 seconds it will return to its original state."
    )
    item:remove(1)
  elseif target.actionid > 4007 and target.actionid < 4024 then
    local graveStorage = storages[target.actionid]
    if
      player:getStorageValue(graveStorage) == 1 or
        player:getStorageValue(Storage.TibiaTales.RestInHallowedGround.Questline) ~= 3
     then
      return false
    end
