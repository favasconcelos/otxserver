...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
  ...
  for i = 1, #config do
    ...
    if not creature or not creature:isPlayer() then
      player:say(
        "You need one player of each vocation having completed the Elemental Spheres quest and also carrying the elemental rare item.",
        TALKTYPE_MONSTER_SAY,
        false,
        0,
        Position(33268, 31835, 10)
      )
      return true
    end
    ...
    if
      vocationId ~= config[i].vocationId or creature:getItemCount(config[i].itemid) < 1 or
        creature:getStorageValue(Storage.ElementalSphere.QuestLine) < 1
     then
      player:say(
        "You need one player of each vocation having completed the Elemental Spheres quest and also carrying the elemental rare item.",
        TALKTYPE_MONSTER_SAY,
