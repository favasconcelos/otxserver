  ...
  elseif item.uid > 3147 and item.uid < 3151 then
    ...
    if player:getStorageValue(reward.storage) ~= 1 then
      player:sendTextMessage(
        ...
        reward.bossName .. " defends his belongings and will not let you open his chest."
      )
      return true
    end
    ...
    if backpack and backpack:getEmptySlots(false) < 5 or player:getFreeCapacity() < 100 then
      player:sendTextMessage(
        MESSAGE_EVENT_ADVANCE,
        "Please make sure that you have at least 5 free inventory slots and that you can carry on additional 100 oz."
      )
      return true
    end
