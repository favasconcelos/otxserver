...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
  ...
  if
    (useItem.time and player:getStorageValue(useItem.storage) > os.time()) or
      player:getStorageValue(useItem.storage) ~= (useItem.formerValue or -1)
   then
  ...
  if player:addItemEx(reward) ~= RETURNVALUE_NOERROR then
    ...
    if player:getFreeCapacity() < weight then
      player:sendCancelMessage(
        "You have found " .. result .. ". Weighing " .. string.format("%.2f", (weight / 100)) .. " oz, it is too heavy."
      )
    else
