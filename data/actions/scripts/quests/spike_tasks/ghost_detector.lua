...
local function getSearchString(fromPos, toPos)
  ...
  return ((type(text[distance]) == "table") and text[distance][level] or text[distance]) ..
    ((distance ~= 0) and dirs[direction] or "")
end
  ...
  if player:getPosition():compare(current) then
    if stat == 2 then
      ...
      player:sendTextMessage(
        ...
        "You found a malignant presence, the glowing detector signals that it does not need any further data."
      )
    else
      GHOST_DETECTOR_MAP[player:getGuid()] = getFreeSand()
      player:sendTextMessage(
        MESSAGE_EVENT_ADVANCE,
        "You found a malignant presence, the glowing detector signals another presence nearby."
      )
    end
    ...
    player:sendTextMessage(
      MESSAGE_EVENT_ADVANCE,
      "The detector points " .. getSearchString(player:getPosition(), current) .. "."
    )
  end
  return true
end
