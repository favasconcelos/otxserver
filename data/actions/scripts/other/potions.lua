local config = {
  ...
  [26030] = {
    health = {min = 400, max = 550},
    mana = {min = 140, max = 250},
    vocations = {3},
    text = "paladins",
    level = 130,
    emptyId = 7635
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
  ...
  if
    (potion.level and player:getLevel() < potion.level) or
      (type(potion.vocations) == "table" and not isInArray(potion.vocations, player:getVocation():getBase():getId())) and
        not (player:getGroup():getId() >= 2)
   then
    player:say(
      string.format("This potion can only be consumed by %s of level %d or higher.", potion.text, potion.level),
  ...
  if
    type(potion.health) == "table" and
      not doTargetCombatHealth(0, target, COMBAT_HEALING, potion.health.min, potion.health.max, CONST_ME_MAGIC_BLUE)
   then
    return false
  end

  if
    type(potion.mana) == "table" and
      not doTargetCombatMana(0, target, potion.mana.min, potion.mana.max, CONST_ME_MAGIC_BLUE)
   then
    return false
  end
