  ...
  if (item.itemid == 13173) then
    if (itemEx.itemid == 11436) then
      if (getPlayerStorageValue(cid, 41600) == 5) and getPlayerStorageValue(cid, 41650) < 3 then
        doCreatureSay(
          cid,
          "You place the trap carefully on the  ground. Between twigs and leaves it is almost invisible.",
          TALKTYPE_ORANGE_1
        )
        ...
        if (getPlayerStorageValue(cid, 41650) == 3) then
          doCreatureSay(
            cid,
            "It looks like the merchants are about to arrive, better hide somewhere where you can see whats going on in the area.",
            TALKTYPE_ORANGE_1
          )
          doRemoveItem(item.uid, 1)
          Game.createItem(13174, 1, toPosition)
