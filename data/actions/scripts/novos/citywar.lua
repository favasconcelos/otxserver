    ...
    if _invite == invite then
      ...
      if player then
        ...
        player:sendTextMessage(
          ...
          'Your city war invite for guild "' .. guildName .. '" has expired.'
        )
      end
        ...
        for _, leaderName in ipairs(invite.invitedLeaders) do
          ...
          if leader then
            leader:sendTextMessage(
              MESSAGE_STATUS_CONSOLE_BLUE,
              'You failed to answer the city war invite from guild "' .. guildName .. '" and it is now expired.'
            )
          end
        end
      end
      ...
      if invite and choice == invite then
        ...
        if ret == CW_RETURNVALUE_TOOFEWPLAYERS then
          return player:sendCancelMessage(
            "Your guild has too few online players to be able to participate in a city war."
          )
        elseif ret == CW_RETURNVALUE_TOOFEWUNIQUEIPS then
          return player:sendCancelMessage(
            "Your guild has too few unique ip addresses to be able to participate in a city war."
          )
        end
