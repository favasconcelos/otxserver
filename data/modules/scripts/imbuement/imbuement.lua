...
local function mergeImbuementList(table1, table2)
  ...
  for i, v in pairs(table1) do
    if
      (v.Name ~= table2.Name and
        not (tableContains(ImbuementElements, v.Type) and tableContains(ImbuementElements, table2.Type)))
     then
...
function Player.applyImbuement(self, msg)
  if (not haveImbuingShrine(self)) then
    sendImbuementError(
      ...
      ErrorMessages.MESSAGEDIALOG_IMBUEMENT_ERROR
    )
    return false
  end
  ...
  if not item then
    sendImbuementError(
      self,
      "Cannot find item, please contact an Administrator.",
      ErrorMessages.MESSAGEDIALOG_IMBUEMENT_ERROR
    )
    return false
  end
  ...
  if (not imbuementNow) then
    sendImbuementError(
      self,
      "Cannot find imbuement data, please contact an Administrator.",
      ErrorMessages.MESSAGEDIALOG_IMBUEMENT_ERROR
    )
    return false
  end
  ...
  if (not self:removeMoneyNpc(imbuingPrice)) then
    sendImbuementError(
      self,
      "You don't have enough money " .. imbuingPrice .. " gps.",
      ErrorMessages.MESSAGEDIALOG_IMBUEMENT_ROLL_FAILED
    )
    return false
  end
  ...
  item:setSpecialAttribute(
    slot,
    imbuementNow.Levels[imbuingLevel] .. " " .. imbuementNow.Name,
    slot + 3,
    72000,
    slot + 6,
    0
...
function Player.clearImbuement(self, msg)
  ...
  if (item == nil) then
    sendImbuementError(
      self,
      "Cannot find item, please send this message to a Administrator.",
      ErrorMessages.MESSAGEDIALOG_CLEARING_CHARM_ERROR
    )
    return false
  end
  ...
  for i = 1, imbuingSlots do
    if (item:isActiveImbuement(i + 3)) then
      ...
      local newDescription =
        existsImbuement.Description:gsub(" %%", " " .. existsImbuement.LevelsPercent[enchantLevel] .. "%%")
