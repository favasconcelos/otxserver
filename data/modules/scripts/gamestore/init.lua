...
function parseTransferCoins(playerId, msg)
  ...
  local resultId =
    db.storeQuery("SELECT `account_id` FROM `players` WHERE `name` = " .. db.escapeString(reciver:lower()) .. "")
  ...
  GameStore.insertHistory(
    ...
    player:getName() .. " transfered you this amount.",
    amount
  )
  GameStore.insertHistory(
    player:getAccountId(),
    GameStore.HistoryTypes.HISTORY_TYPE_NONE,
    "You transfered this amount to " .. reciver,
    -1 * amount
  ) -- negative
end
...
function parseBuyStoreOffer(playerId, msg)
  ...
  if offer then
    ...
    if not offer.type or offer.type == GameStore.OfferTypes.OFFER_TYPE_NONE then
      return addPlayerEvent(
        ...
        250,
        playerId,
        GameStore.StoreErrors.STORE_ERROR_NETWORK,
        "The offer is either fake or corrupt."
      )
    end
    ...
    -- If no thing id,
    if
      offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and
    ...
    if not player:canRemoveCoins(newPrice or offer.price) then
      return addPlayerEvent(
        ...
        playerId,
        GameStore.StoreErrors.STORE_ERROR_NETWORK,
        "We couldn't remove coins from your account, try again later."
      )
    end
    ...
    local message =
      "You have purchased " .. offerCountStr .. offer.name .. " for " .. (newPrice or offer.price) .. " coins."
    ...
    elseif
      offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT or offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT_ADDON
     then
      -- If offer is mount
      ...
      if not outfitLookType then
        return addPlayerEvent(
          ...
          playerId,
          GameStore.StoreErrors.STORE_ERROR_NETWORK,
          "This outfit seems not to suit your sex, we are sorry for that!"
        )
      end
    ...
    elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_TEMPLE then
      if (player:getCondition(CONDITION_INFIGHT) or player:isPzLocked()) then
        return addPlayerEvent(
          ...
          playerId,
          GameStore.StoreErrors.STORE_ERROR_NETWORK,
          "You can't use temple teleport in fight!"
        )
      end
    ...
    elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PROMOTION then
      ...
      return addPlayerEvent(
        ...
        playerId,
        GameStore.StoreErrors.STORE_ERROR_NETWORK,
        "This offer is fake, please contact admin."
      )
    end
    -- Removing coins
    ...
    GameStore.insertHistory(
      player:getAccountId(),
      GameStore.HistoryTypes.HISTORY_TYPE_NONE,
      offerCountStr .. offer.name,
      (newPrice or offer.price) * -1
    )
  ...
  addPlayerEvent(
    ...
    "We couldn't locate this offer, please try again later."
  )
end
      ...
      if disabled ~= 1 then
        ...
        if offer.type == GameStore.OfferTypes.OFFER_TYPE_BLESSINGS then
          if player:hasBlessing(offer.thingId) and offer.thingId < 9 then
            ...
            if
              player:hasBlessing(1) and player:hasBlessing(2) and player:hasBlessing(3) and player:hasBlessing(4) and
                player:hasBlessing(5) and
                player:hasBlessing(6) and
                player:hasBlessing(7) and
                player:hasBlessing(8)
             then
              disabled = 1
        ...
        if
          offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT or
            offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT_ADDON
         then
          local outfitLookType
          if player:getSex() == PLAYERSEX_MALE then
            outfitLookType = offer.thingId.male
...
function sendStoreTransactionHistory(playerId, page, entriesPerPage)
  ...
  if #entries == 0 then
    return addPlayerEvent(
      ...
      250,
      playerId,
      GameStore.StoreErrors.STORE_ERROR_HISTORY,
...
GameStore.insertHistory = function(accountId, mode, description, amount)
  return db.query(
    string.format(
      "INSERT INTO `store_history`(`account_id`, `mode`, `description`, `coin_amount`, `time`) VALUES (%s, %s, %s, %s, %s)",
      accountId,
      mode,
...
GameStore.retrieveHistoryEntries = function(accountId)
  ...
  local resultId =
    db.storeQuery(
    "SELECT * FROM `store_history` WHERE `account_id` = " .. accountId .. " ORDER BY `time` DESC LIMIT 15;"
  )
  if resultId ~= false then
    repeat
      local entry = {
  ...
  if not Vocation(newVocId) then
    if send then
      addPlayerEvent(
        ...
        playerId,
        GameStore.StoreErrors.STORE_ERROR_NETWORK,
        "The offer is fake, please report it!"
      )
    end
  ...
  if newVocId <= vocationCopy:getId() then
    if send then
      addPlayerEvent(
        ...
        playerId,
        GameStore.StoreErrors.STORE_ERROR_NETWORK,
        "You already have this promotion!"
      )
    end
    result.ability = false
    return result
  ...
  if (newVocId - baseVocationsCount) ~= vocationCopy:getId() then
    if send then
      addPlayerEvent(
        ...
        playerId,
        GameStore.StoreErrors.STORE_ERROR_NETWORK,
        "You need higher promotion to get his one."
      )
    end
    result.ability = false
    return result
...
function Player.addCoinsBalance(self, coins, update)
  ...
  if update then
    sendCoinBalanceUpdating(self, true)
  end
  return true
end
