...
local function havePreyMonsterByName(playerId, name)
  return db.storeQuery(
    "SELECT player_id FROM player_prey WHERE player_id = " .. playerId .. " AND name = " .. db.escapeString(name)
  )
end
  ...
  repeat
    ...
    if (not havePreyMonsterByName(player:getGuid(), randomName)) then
      if (MonsterType(randomName) and MonsterType(randomName):getOutfit().lookType > 0) then
        ...
        db.query(
          "INSERT INTO player_prey SET player_id = " ..
            player:getGuid() ..
              ", name = " .. db.escapeString(randomName) .. ", mindex = " .. (count - 1) .. ", mcolumn = " .. column
        )
        count = count + 1
      end
    end
...
local function getMonsterName(player, column, index)
  local resultId =
    db.storeQuery(
    "SELECT name FROM player_prey WHERE mcolumn = " ..
      column .. " AND mindex = " .. index .. " AND player_id = " .. player:getGuid()
  )
...
function CheckPrey(player, msg)
  ...
  elseif (PreyAction == 1) then
    if (player:isBonusReroll(PreyColumn)) then
      return sendError(
        player,
        "Are you fucking kidding me? Do you really wanna try bug my system? Get out of here little sniffer."
      )
    end
...
function Player.removePreyMonster(self, name)
  local resultId =
    db.storeQuery(
    "SELECT * FROM player_prey WHERE player_id = " .. self:getGuid() .. " AND name = " .. db.escapeString(name)
  )
  if (resultId ~= false) then
    local lastIndex = 0
    local mIndex = result.getDataInt(resultId, "mindex")
    ...
    local totalElements =
      db.storeQuery(
      "SELECT COUNT(*) FROM player_prey WHERE player_id = " .. self:getGuid() .. " AND mcolumn = " .. mColumn
    )
    if (totalElements ~= false) then
      lastIndex = result.getDataInt(totalElements, "COUNT(*)") - 1
    end
    if (mIndex == lastIndex) then
      return true
    end

    local resultMonsters =
      db.storeQuery(
      "SELECT * FROM player_prey WHERE player_id = " ..
        self:getGuid() .. " AND mcolumn = " .. mColumn .. " AND mindex > " .. mIndex .. " AND mindex <= " .. lastIndex
    )
    repeat
      local tmpIndex = result.getDataInt(resultMonsters, "mindex")
      db.query(
        "UPDATE player_prey SET mindex = mindex - 1 WHERE player_id = " ..
          self:getGuid() .. " AND mcolumn = " .. mColumn .. " AND mindex = " .. tmpIndex
      )
