...
function onWrap(player, positionItem, itemId)
  ...
  if (getDistanceBetween(positionItem.x, playerPos.x) > 1 or getDistanceBetween(positionItem.y, playerPos.y) > 1) then
  ...
  if (decoItem:isContainer() and Container(decoItem:getUniqueId()):getItemHoldingCount() > 0) then
