...
function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "spike") then
    return npcHandler:say(
      "Now that's gnomish ingenuity given shape! Who but a gnome would come up with such a plan to defeat our enemies. ",
      cid
    )
  end
  ...
  if msgcontains(msg, "mission") then
    if player:getLevel() < level then
      npcHandler:say(
        "Sorry, but no! Your expertise could be put to better use elsewhere. You are desperately needed in the upper levels of the Spike. Report there immediately. ",
        cid
      )
    else
      npcHandler:say(
        "I can offer you several missions: to {deliver} parcels to our boys and girls in the battlefield, to get reports from our {undercover} gnomes, to do some {temperature} measuring and to {kill} some drillworms.",
        cid
      )
    end
    return
  ...
  if msgcontains(msg, "report") then
    ...
    return npcHandler:say(
      " What mission do you want to report about: the {delivery} of parcels, the {undercover} reports, the {temperature} measuring or {kill} of drillworms?",
      cid
    )
  end
  ...
  if talkState[cid] == "report" then
    ...
    elseif msgcontains(msg, "undercover") then
      ...
      elseif player:getStorageValue(SPIKE_LOWER_UNDERCOVER_MAIN) == 3 then
        ...
        npcHandler:say(
          "Gnowful! Get three reports from our undercover agents posing as monsters in the caves around us.",
          cid
        )
      end
    elseif msgcontains(msg, "temperature") then
      ...
      elseif player:getStorageValue(SPIKE_LOWER_LAVA_MAIN) == 1 then
        ...
        npcHandler:say(
          "Gnowful! Use the gnomish temperature measurement device to locate the hottest spot at the lava pools in the cave.",
          cid
        )
      end
  ...
  if msgcontains(msg, "deliver") then
    if player:getExhaustion(SPIKE_LOWER_PARCEL_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_LOWER_PARCEL_DAILY) - os.time()) ..
    ...
    if player:getStorageValue(SPIKE_LOWER_PARCEL_MAIN) == -1 then
      npcHandler:say(
        "We need someone to bring four parcels to some of our far away outposts in the caverns. If you are interested, I can give you some more {information} about it. Are you willing to accept this mission?",
        cid
      )
  ...
  if msgcontains(msg, "undercover") then
    if player:getExhaustion(SPIKE_LOWER_UNDERCOVER_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_LOWER_UNDERCOVER_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
  ...
  if talkState[cid] == "undercover" then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Gnometastic! Get three reports from our agents. You can find them anywhere in the caves around us. Just keep looking for monsters that behave strangely and give you a wink.",
        cid
      )
      talkState[cid] = nil
    elseif msgcontains(msg, "no") then
  ...
  if msgcontains(msg, "temperature") then
    if player:getExhaustion(SPIKE_LOWER_LAVA_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_LOWER_LAVA_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
  ...
  if msgcontains(msg, "kill") then
    if player:getExhaustion(SPIKE_LOWER_KILL_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_LOWER_KILL_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_LOWER_KILL_MAIN) == -1 then
      npcHandler:say(
        "This mission will require you to kill some drillworms for us. If you are interested, I can give you some more {information} about it. Are you willing to accept this mission?",
        cid
      )
      talkState[cid] = "kill"
    else
