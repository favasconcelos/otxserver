...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "join") then
    if
      player:getStorageValue(Storage.OutfitQuest.NightmareOutfit) < 1 and
        player:getStorageValue(Storage.OutfitQuest.BrotherhoodOutfit) < 1
     then
      npcHandler:say(
        {
  ...
  elseif msgcontains(msg, "advancement") then
    ...
    elseif player:getStorageValue(Storage.OutfitQuest.BrotherhoodOutfit) == 2 then
      npcHandler:say(
        "So you want to advance to a {Death Dealer} rank? Did you bring 1000 demonic essences with you?",
        cid
      )
      npcHandler.topic[cid] = 4
    elseif player:getStorageValue(Storage.OutfitQuest.BrotherhoodOutfit) == 3 then
      npcHandler:say(
        "So you want to advance to a {Dread Lord} rank? Did you bring 1500 demonic essences with you?",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      if player:removeItem(6500, 500) then
        ...
        npcHandler:say(
          "You advanced to {Hyaena} rank! You are now able to use teleports of second floor of Knightwatch Tower.",
          cid
        )
      else
    ...
    elseif npcHandler.topic[cid] == 5 then
      if player:removeItem(6500, 1500) then
        ...
        npcHandler:say(
          "You advanced to {Dread Lord} rank! You are now able to use teleports of fourth floor of Knightwatch Tower and to create addon scrolls.",
          cid
        )
      else
        npcHandler:say("Come back when you gather all essences.", cid)
