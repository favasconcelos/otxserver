...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "mission") then
    if player:getStorageValue(Storage.ExplorerSociety.QuestLine) == 56 then
      npcHandler:say(
        "Ah, you've just come in time. An experienced explorer is just what we need here! Would you like to go on a mission for us?",
        cid
      )
    ...
    elseif
      player:getStorageValue(Storage.TheIceIslands.Questline) == 34 and
        player:getStorageValue(Storage.TheIceIslands.MemoryCrystal) > os.time()
     then
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      if player:removeItem(7281, 1) then
        npcHandler:say(
          "Ah, great. Please give me some time to evaluate the information. Then talk to me again about your mission. ",
          cid
        )
