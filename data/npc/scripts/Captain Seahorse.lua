...
local function addTravelKeyword(keyword, cost, destination, action)
  local travelKeyword =
    ...
    StdModule.say,
    {
      ...
      text = "Do you seek a seek a passage to " .. keyword:titleCase() .. " for |TRAVELCOST|?",
      cost = cost,
      discount = "postman"
    }
  )
...
keywordHandler:addKeyword(
  ...
  {
    npcHandler = npcHandler,
    text = "Where do you want to go? To {Krailos}, {Thais}, {Carlin}, {Ab'Dendriel}, {Venore}, {Port Hope}, {Ankrahmun}, {Liberty Bay}, {Gray Island} or the isle {Cormaya}?"
  }
)
keywordHandler:addKeyword(
  {"passage"},
...
keywordHandler:addKeyword(
  {"name"},
  StdModule.say,
  {npcHandler = npcHandler, text = "My name is Captain Seahorse from the Royal Tibia Line."}
)
keywordHandler:addKeyword(
  {"edron"},
  StdModule.say,
  {npcHandler = npcHandler, text = "This is Edron. Where do you want to go?"}
)
keywordHandler:addKeyword(
  {"yalahar"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "I'm sorry, but we don't serve this route. However, I heard that Wyrdin here in Edron is looking for adventurers to go on a trip to Yalahar for him."
  }
)
