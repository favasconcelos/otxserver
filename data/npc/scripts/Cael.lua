...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "tome") or msgcontains(msg, "knowledge") then
    ...
    if player:getStorageValue(Storage.TheNewFrontier.Mission08) == 2 then
      ...
      elseif
        player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) >= 1 and
          player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) <= 5
       then --tome2 - tome6
      ...
      elseif
        player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) >= 6 and
          player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) <= 11
       then --tome7 - tome12
      ...
      elseif player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) >= 12 then -- more then 12 tomes
        npcHandler:say(
          "Oh! That sounds fascinating. Have you found a Tome of Knowledge for me to read? I have the feeling though that I can only share some of my experience with you now. Is that alright with you?",
          cid
        )
  ...
  elseif msgcontains(msg, "yes") and npcHandler.topic[cid] >= 1 and npcHandler.topic[cid] <= 13 then
    if player:removeItem(11134, 1) then --remove tome
      if npcHandler.topic[cid] == 1 then --tome1
        npcHandler:say(
          "Thank you! I look forward to reading this interesting discovery of yours and learn a few things about {Zao}.",
          cid
        )
      ...
      elseif npcHandler.topic[cid] >= 2 and npcHandler.topic[cid] <= 12 then --tome2 - tome12
        ...
        player:setStorageValue(
          Storage.TheNewFrontier.TomeofKnowledge,
          player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) + 1
        )
      ...
      elseif npcHandler.topic[cid] == 13 then -- more then 12 tomes
        ...
        npcHandler:say(
          "Thank you! I look forward to reading this interesting discovery of yours and learn a few things about {Zao}. Let me share some experience with you.",
          cid
        )
  ...
  elseif (msgcontains(msg, "zao")) then
    ...
    elseif npcHandler.topic[cid] == 24 then --tome4
      npcHandler:say(
        "I've learnt something interesting about a certain {food} that the lizardmen apparently prepare.",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 28 then --tome8
      npcHandler:say(
        "I've learnt a few things about an illness, or how I prefer to call it, {corruption} of this land.",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 30 then --tome10
      npcHandler:say(
        "This book actually IS about Zao. Not about the continent, but about the mythical {founder} of the lizard dynasty.",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 32 then --tome12
      npcHandler:say(
        "The last tome contained a lot of information about status symbols and insignia - such as {thrones} - and reveals some of the power structures in Zao.",
        cid
      )
      npcHandler.topic[cid] = 51
    elseif npcHandler.topic[cid] == 33 then --more than tome12
      npcHandler:say(
        "I've learnt many things from your books. Still, I guess that's just a fragment of what I could still discover about this interesting continent.",
        cid
      )
      npcHandler.topic[cid] = 0
    end
  ...
  elseif msgcontains(msg, "yes") and npcHandler.topic[cid] == 65 then
    if player:removeItem(11206, 1) then
      ...
      npcHandler:say(
        "Let's put this little lantern here.. there you go. I wrap it up for you, just unwrap it in your house again!",
        cid
      )
      npcHandler.topic[cid] = 0
    else
  ...
  elseif msgcontains(msg, "cloth") then
    if player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) >= 12 then
      npcHandler:say(
        "Have you brought me a piece of red cloth? I can make that throne for you if you want. But remember, I won't do that all the time - so try and don't destroy it, okay?",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") and npcHandler.topic[cid] == 66 then
    if player:removeItem(5911, 1) then
      ...
      npcHandler:say(
        "Let's put this cloth over the seat.. there you go. I wrap it up for you, just unwrap it in your house again!",
        cid
      )
      npcHandler.topic[cid] = 0
    else
  ...
  elseif msgcontains(msg, "yes") and npcHandler.topic[cid] >= 60 and npcHandler.topic[cid] <= 61 then
    if npcHandler.topic[cid] == 60 then
      if not player:hasOutfit(player:getSex() == PLAYERSEX_FEMALE and 336 or 335, 1) and player:removeItem(11116, 1) then
        ...
        npcHandler:say(
          "Thank you! Let me reward you with something I stumbled across recently and which might fit your warmaster outfit perfectly.",
          cid
        )
        npcHandler.topic[cid] = 0
      else
  ...
  elseif msgcontains(msg, "trade") then
    if player:getStorageValue(Storage.TheNewFrontier.TomeofKnowledge) >= 6 then
      ...
      local function onBuy(cid, item, subType, amount, ignoreCap, inBackpacks)
        if
          (ignoreCap == false and
            (player:getFreeCapacity() < ItemType(items[item].itemId):getWeight(amount) or
              inBackpacks and
                player:getFreeCapacity() < (ItemType(items[item].itemId):getWeight(amount) + ItemType(1988):getWeight())))
         then
        ...
        if items[item].buyPrice <= player:getMoney() then
          if inBackpacks then
            ...
            return player:addItem(items[item].itemId, amount, false, items[item]) and
              player:removeMoneyNpc(amount * items[item].buyPrice) and
              player:sendTextMessage(
                MESSAGE_INFO_DESCR,
                "You bought " ..
                  amount .. "x " .. items[item].realName .. " for " .. items[item].buyPrice * amount .. " gold coins."
              )
          end
          player:sendTextMessage(
            MESSAGE_INFO_DESCR,
      ...
      local function onSell(cid, item, subType, amount, ignoreEquipped)
        if items[item].sellPrice then
          return player:removeItem(items[item].itemId, amount, -1, ignoreEquipped) and
            player:addMoney(items[item].sellPrice * amount) and
            player:sendTextMessage(
              MESSAGE_INFO_DESCR,
              "You sold " ..
                amount .. "x " .. items[item].realName .. " for " .. items[item].sellPrice * amount .. " gold coins."
            )
        end
        return true
      end
