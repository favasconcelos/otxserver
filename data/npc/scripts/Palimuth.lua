...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "job") and not player:getStorageValue(Storage.InServiceofYalahar.Questline) == 54 then
    npcHandler:say(
      "I'm an Augur of the city of Yalahar. My special duty consists of coordinating the efforts to keep the city and its services running.",
      cid
    )
  ...
  elseif msgcontains(msg, "mission") then
    ...
    elseif
      player:getStorageValue(Storage.InServiceofYalahar.Questline) >= 7 and
        player:getStorageValue(Storage.InServiceofYalahar.Questline) <= 14
     then
    ...
    elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 15 then
      ...
      npcHandler.topic[cid] = 0
    elseif
      player:getStorageValue(Storage.InServiceofYalahar.NotesPalimuth) == 1 and
        player:getStorageValue(Storage.InServiceofYalahar.Questline) == 16
     then
      npcHandler:say(
        {
    ...
    elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 23 then
      ...
      npcHandler.topic[cid] = 0
    elseif
      player:getStorageValue(Storage.InServiceofYalahar.Questline) == 25 and
        player:getStorageValue(Storage.InServiceofYalahar.MrWestStatus) == 1
     then
      npcHandler:say(
        {
      ...
      player:setStorageValue(
        Storage.InServiceofYalahar.GoodSide,
        player:getStorageValue(Storage.InServiceofYalahar.GoodSide) >= 0 and
          player:getStorageValue(Storage.InServiceofYalahar.GoodSide) + 1 or
          0
      )
    ...
    elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 28 then
      ...
      npcHandler.topic[cid] = 0
    elseif
      player:getStorageValue(Storage.InServiceofYalahar.Questline) == 32 and
        player:getStorageValue(Storage.InServiceofYalahar.TamerinStatus) == 1
     then
      npcHandler:say(
        "These are great news indeed. The people of Yalahar will be grateful. The Yalahari probably not, so take care of yourself. ",
        cid
      )
      player:setStorageValue(
        Storage.InServiceofYalahar.GoodSide,
    ...
    elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 40 then
      ...
      npcHandler.topic[cid] = 0
    elseif
      player:getStorageValue(Storage.InServiceofYalahar.Questline) == 42 and
        player:getStorageValue(Storage.InServiceofYalahar.QuaraState) == 1
     then
      npcHandler:say(
        "Oh no! So that's the reason for the quara attacks! I will do my best to close these sewage pipes. We will have to use other drains. ",
        cid
      )
    ...
    elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 48 then
      ...
      npcHandler.topic[cid] = 0
    elseif
      player:getStorageValue(Storage.InServiceofYalahar.Questline) == 49 or
        player:getStorageValue(Storage.InServiceofYalahar.Questline) == 48
     then
      npcHandler:say("So do you want to side with me? ", cid)
      npcHandler.topic[cid] = 5
    elseif
      player:getStorageValue(Storage.InServiceofYalahar.Questline) == 50 and
        player:getStorageValue(Storage.InServiceofYalahar.SideDecision) == 1
     then
      npcHandler:say(
        {
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 2 then
      if
        player:getStorageValue(Storage.InServiceofYalahar.SewerPipe01) == 1 and
          player:getStorageValue(Storage.InServiceofYalahar.SewerPipe02) == 1 and
          player:getStorageValue(Storage.InServiceofYalahar.SewerPipe03) == 1 and
          player:getStorageValue(Storage.InServiceofYalahar.SewerPipe04) == 1
       then
        ...
        npcHandler:say(
          "Thank you very much. You have no idea how hard it was to find someone volunteering for that job. If you feel ready for further {missions}, just tell me.",
          cid
        )
        npcHandler.topic[cid] = 0
      end
    ...
    elseif npcHandler.topic[cid] == 4 then
      if player:getStorageValue(Storage.InServiceofYalahar.Questline) == 14 then
        ...
        npcHandler:say(
          "Excellent! My superiors will be pleased to get these reports. I will for sure emphasise your efforts in this mission. Please come back soon to see if there are any more {missions} available for you. ",
          cid
        )
        npcHandler.topic[cid] = 0
      else
    ...
    elseif npcHandler.topic[cid] == 5 then
      ...
      npcHandler:say(
        "I knew that you were smart enough to make the right decision! Your next mission will be a special one! ",
        cid
      )
      npcHandler.topic[cid] = 0
