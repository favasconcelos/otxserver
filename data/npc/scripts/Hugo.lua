...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "uniforms") then
    if player:getStorageValue(Storage.postman.Mission06) == 1 then
      npcHandler:say(
        "A new uniform for the post officers? I am sorry but my dog ate the last dress pattern we used. You need to supply us with a new dress pattern.",
        cid
      )
  ...
  elseif msgcontains(msg, "dress pattern") then
    if npcHandler.topic[cid] == 1 then
      npcHandler:say(
        "It was ... wonderous beyond wildest imaginations! I have no clue where Kevin Postner got it from. Better ask him.",
        cid
      )
      player:setStorageValue(Storage.postman.Mission06, 2)
    elseif player:getStorageValue(Storage.postman.Mission06) == 11 then
      npcHandler:say(
        "By the gods of fashion! Didn't it do that I fed the last dress pattern to my poor dog? Will this mocking of all which is taste and fashion never stop?? Ok, ok, you will get those ugly, stinking uniforms and now get lost, fashion terrorist.",
        cid
      )
  ...
  elseif msgcontains(msg, "outfit") then
    ...
    elseif
      player:getStorageValue(Storage.OutfitQuest.BeggarOutfit) > 0 and
        player:getStorageValue(Storage.OutfitQuest.BeggarOutfit) < 5
     then
      npcHandler:say(
        "I am so excited! This poor man's look will be an outfit like the world has never seen before.",
        cid
      )
    elseif player:getStorageValue(Storage.OutfitQuest.BeggarOutfit) == 5 then
      ...
      elseif
        player:getStorageValue(Storage.OutfitQuest.BeggarOutfitTimer) > 0 and
          player:getStorageValue(Storage.OutfitQuest.BeggarOutfitTimer) < os.time()
       then
        npcHandler:say(
          "Eureka! Alas, the poor man's outfit is finished, but... to be honest... it turned out much less appealing than I expected. However, you can have it if you want, okay?",
          cid
        )
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      ...
      npcHandler:say(
        "Terrific! What are you waiting for?! Start right away gathering 20 pieces of brown cloth and come back once you have them!",
        cid
      )
      npcHandler.topic[cid] = 0
    elseif npcHandler.topic[cid] == 4 then
      ...
      player:setStorageValue(
        Storage.OutfitQuest.BeggarOutfit,
        player:getStorageValue(Storage.OutfitQuest.BeggarOutfit) + 1
      )
