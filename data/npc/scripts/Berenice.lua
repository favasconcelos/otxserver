...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "mission") then
    if player:getStorageValue(Storage.ExplorerSociety.CalassaQuest) == 2 then
      npcHandler:say(
        "OH! So you have safely returned from Calassa! Congratulations, were you able to retrieve the logbook?",
        cid
      )
  ...
  elseif msgcontains(msg, "calassa") then
    if npcHandler.topic[cid] == 1 and player:getStorageValue(Storage.ExplorerSociety.CalassaQuest) < 1 then
      npcHandler:say(
        "Ah! So you have heard about our special mission to investigate the Quara race in their natural surrounding! Would you like to know more about it?",
        cid
      )
      npcHandler.topic[cid] = 2
    elseif npcHandler.topic[cid] == 4 then
      npcHandler:say(
        "Captain Max will bring you to Calassa whenever you are ready. Please try to retrieve the missing logbook which must be in one of the sunken shipwrecks.",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      npcHandler:say(
        "Excellent! I will immediately inform Captain Max to bring you to {Calassa} whenever you are ready. Don't forget to make thorough preparations!",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 5 then
      if player:removeItem(6124, 1) then
        ...
        npcHandler:say(
          "Yes! That's the logbook! However... it seems that the water has already destroyed many of the pages. This is not your fault though, you did your best. Thank you!",
          cid
        )
        npcHandler.topic[cid] = 0
      end
