...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "mission") then
    ...
    elseif player:getStorageValue(Storage.toOutfoxAFoxQuest) == 1 then
      if player:removeItem(7497, 1) then
        ...
        npcHandler:say(
          "I always said it to the others 'This brave fellow will bring me my mining helmet back' and here you are with it!! Here take my spare helmet, I don't need it anymore!",
          cid
        )
      else
        npcHandler:say(
          "We presume the hideout of The Horned Fox is somewhere in the south-west near the coast, good luck finding my mining helmet!",
          cid
        )
      end
      npcHandler.topic[cid] = 0
    else
      npcHandler:say("Hum... what, {task}?", cid)
    end
  elseif msgcontains(msg, "yes") then
    if npcHandler.topic[cid] == 1 then
      player:setStorageValue(Storage.toOutfoxAFoxQuest, 1)
      npcHandler:say(
        "I knew you have the guts for that task! We presume the hideout of The Horned Fox somewhere in the south-west near the coast. Good luck!",
        cid
      )
      npcHandler.topic[cid] = 0
    elseif npcHandler.topic[cid] == 2 then
      npcHandler:say(
        "Hussah! Let's bring war to those hoof-legged, dirt-necked, bull-headed minotaurs!! Come back to me when you are done with your mission.",
        cid
      )
  ...
  elseif msgcontains(msg, "task") then
    ...
    elseif player:getStorageValue(Storage.KillingInTheNameOf.BudrikMinos) == 3 then
      ...
      npcHandler:say("You need to do the {To Outfox a Fox Quest} before.", cid)
    end
