...
local function greetCallback(cid)
  ...
  if player:getStorageValue(Storage.HotCuisineQuest.QuestStart) < 1 then
    ...
    npcHandler:setMessage(
      MESSAGE_GREET,
      "Hello there again, |PLAYERNAME|! I guess you're back for some cooking - let's get going then!"
    )
  end
  return true
end
...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "cook") then
    if player:getStorageValue(Storage.HotCuisineQuest.QuestStart) < 1 then
      npcHandler:say(
        "Well, I'm not a simple cook. I travel the whole Tibian continent for the most artfully seasoned {recipes} and constantly develop new ones.",
        cid
      )
  ...
  elseif msgcontains(msg, "recipe") or msgcontains(msg, "menu") then
    if npcHandler.topic[cid] == 1 then
      npcHandler:say(
        "You're interested in my recipes? Well. They are not for sale, but if you want to become my {apprentice}, I'll share my knowledge with you.",
        cid
      )
  ...
  elseif msgcontains(msg, "apprentice") then
    if npcHandler.topic[cid] == 2 then
      npcHandler:say(
        "Hmm. You look quite promising. Can't hurt to give it a try, I guess. Would you like to become my apprentice, |PLAYERNAME|?",
        cid
      )
