  ...
  if msgcontains(msg, "death portal") then
    if player:getStorageValue(50730) == 1 then
      if player:getStorageValue(items.item1[2]) <= 0 and getPlayerItemCount(cid, items.item1[1]) >= counts.count1[1] then
        ...
        selfSay(
          "You may pass this portal if you have 50 pelvis bones as offering. Do you have the bones with you?",
          cid
        )
      end
    else
  ...
  elseif msgcontains(msg, "energy portal") then
    if player:getStorageValue(50730) == 1 then
      if player:getStorageValue(items.item2[2]) <= 0 and getPlayerItemCount(cid, items.item2[1]) >= counts.count1[1] then
        ...
        selfSay(
          "You may pass this portal if you have 50 marsh stalker feathers as offering. Do you have the feathers with you?",
          cid
        )
      end
    else
  ...
  elseif msgcontains(msg, "holy portal") then
    if player:getStorageValue(50730) == 1 then
      if player:getStorageValue(items.item5[2]) <= 0 and getPlayerItemCount(cid, items.item5[1]) >= counts.count1[1] then
        ...
        selfSay(
          "You may pass this portal if you have 50 incantation notes as offering. Do you have the notes with you?",
          cid
        )
      end
    else
...
keywordHandler:addKeyword(
  ...
  StdModule.say,
  {
    ...
    text = "I find ways to unveil the secrets of the stars. Judging by this question, I doubt you follow my weekly publications concerning this research."
  }
)
keywordHandler:addKeyword(
  {"name"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I am Albinius, a worshipper of the {Astral Shapers}."}
)

npcHandler:setCallback(CALLBACK_MESSAGE_DEFAULT, creatureSayCallback)
npcHandler:setMessage(
  MESSAGE_GREET,
  "Greetings, pilgrim. Welcome to the halls of hope. We are the keepers of this {temple} and welcome everyone willing to contribute."
)
