...
local function creatureSayCallback(cid, type, msg)
    ...
    elseif msgcontains(msg, "cookie") then
      if
        player:getStorageValue(Storage.WhatAFoolishQuest.Questline) == 31 and
          player:getStorageValue(Storage.WhatAFoolishQuest.CookieDelivery.AvarTar) ~= 1
       then
  ...
  elseif msgcontains(msg, "yes") then
    if npcHandler.topic[cid] == 1 then
      npcHandler:say(
        "So you want to have the demon outfit, hah! Let's have a look first if you really deserve it. Tell me: {base}, {shield} or {helmet}?",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 3 then
      ...
      npcHandler:say(
        "Well, you won't! Though it looks tasty ...What the ... WHAT DO YOU THINK YOU ARE? THIS IS THE ULTIMATE INSULT! GET LOST!",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 2 then
    ...
    elseif msgcontains(msg, "shield") then
      if
        player:getStorageValue(Storage.AnnihilatorDone) == 2 and
          player:getStorageValue(Storage.QuestChests.DemonHelmetQuestDemonHelmet) == 1
       then
        ...
        npcHandler:say(
          "The shield will only be granted to those adventurers who have finished the demon helmet quest, |PLAYERNAME|.",
          cid
        )
        npcHandler.topic[cid] = 2
      end
    elseif msgcontains(msg, "helmet") then
      if player:getStorageValue(Storage.AnnihilatorDone) == 2 and player:getStorageValue(Storage.DemonOak.Done) == 3 then
        ...
        npcHandler:say(
          "The helmet is for those who have hunted down all 6666 demons and finished the demon oak as well, |PLAYERNAME|.",
          cid
        )
        npcHandler.topic[cid] = 2
      end
