...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "food") then
    ...
    elseif player:getStorageValue(Storage.Oramond.MissionToTakeRoots) == 1 then
      if player:getStorageValue(Storage.Oramond.HarvestedRootCount) < 5 then
        npcHandler:say(
          "I am sorry, you didn't harvest enough roots. You need to harvest a bundle of at least five roots - and please try doing it yourself.",
          cid
        )
  ...
  elseif msgcontains(msg, "root") then
    npcHandler:say(
      "They are nutritious, cost nothing and are good for the body hair. If you can bring us bundles of five juicy roots each - we will make it worth your while for the {magistrate}.",
      cid
    )
  elseif msgcontains(msg, "magistrate") then
    npcHandler:say(
      "They act so important but it is us common people who keep things going. There is a lot you can do in this city to earn a right to vote in the magistrate, though. So keep an eye out for everyone who needs help.",
      cid
    )
...
npcHandler:setMessage(
  MESSAGE_GREET,
  "Hey there! You don't happen to have some {food} on you, you're willing to share? Well, where are my manners, a warm welcome for now."
)
