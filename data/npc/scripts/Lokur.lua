...
local function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, "yes") then
      npcHandler:say(
        "Alright, we have placed an order to deposit the amount of " ..
          count[cid] .. " gold to your guild account. Please check your inbox for confirmation.",
        cid
      )
    ...
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "We placed an order to withdraw " ..
          count[cid] .. " gold from your guild account. Please check your inbox for confirmation.",
        cid
      )
      local info = {
        type = "Guild Withdraw",
        ...
        if transfer[cid] then
          npcHandler:say(
            "So you would like to transfer " ..
              count[cid] .. " gold from your guild account to guild " .. transfer[cid] .. "?",
            cid
          )
    ...
    if msgcontains(msg, "yes") then
      npcHandler:say(
        "We have placed an order to transfer " ..
          count[cid] ..
            " gold from your guild account to guild " .. transfer[cid] .. ". Please check your inbox for confirmation.",
        cid
      )
      local guild = player:getGuild()
      local balance = guild:getBankBalance()
  ...
  if msgcontains(msg, "ticket") then
    if player:getStorageValue(Storage.wagonTicket) < os.time() then
      npcHandler:say(
        "Do you want to purchase a weekly ticket for the ore wagons? With it you can travel freely and swiftly through Kazordoon for one week. 250 gold only. Deal?",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") then
    if npcHandler.topic[cid] == 29 then
      if player:removeMoneyNpc(250) then
        ...
        npcHandler:say(
          "Here is your stamp. It can't be transferred to another person and will last one week from now. You'll get notified upon using an ore wagon when it isn't valid anymore.",
          cid
        )
      else
  ...
  elseif msgcontains(msg, "measurements") then
    if player:getStorageValue(Storage.postman.Mission07) >= 1 then
      npcHandler:say(
        "Come on, I have no clue what they are. Better ask my armorer Kroox for such nonsense.Go and ask him for good ol' Lokurs measurements, he'll know.",
        cid
      )
    end
  end
...
keywordHandler:addKeyword(
  {"money"},
  StdModule.say,
  {npcHandler = npcHandler, text = "We can {change} money for you. You can also access your {bank account}."}
)
keywordHandler:addKeyword(
  {"change"},
  StdModule.say,
  {
    ...
    text = "There are three different coin types in Tibia: 100 gold coins equal 1 platinum coin, 100 platinum coins equal 1 crystal coin. So if you'd like to change 100 gold into 1 platinum, simply say '{change gold}' and then '1 platinum'."
  }
)
keywordHandler:addKeyword(
  {"bank"},
