...
local function greetCallback(cid)
  ...
  if player:getStorageValue(Storage.OutfitQuest.MageSummoner.AddonWand) < 1 then
    npcHandler:setMessage(
      MESSAGE_GREET,
      "The gods must be praised that I am finally saved. I do not have many worldly possessions, but please accept a small reward, do you?"
    )
...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "Yes") then
    if player:getStorageValue(Storage.OutfitQuest.MageSummoner.AddonWand) < 1 then
      npcHandler:say(
        "OI will tell you a small secret now. My friend Lynda in Thais can create a blessed wand. Greet her from me, maybe she will aid you.",
        cid
      )
