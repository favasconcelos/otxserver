  ...
  if playerStatus == PROPACCEPT_STATUS then
    ...
    delayedSay(
      "Dear friends and family, we are gathered here today to witness and celebrate the union of " ..
        getPlayerNameById(candidate) .. " and " .. player:getName() .. " in marriage."
    )
    delayedSay(
    ...
    delayedSay(
      "*After a whispered blessing opens an hand towards " ..
        player:getName() .. "* Take these two engraved wedding rings and give one of them to your spouse.",
      22000
    )
...
local function confirmRemoveEngage(cid, message, keywords, parameters, node)
  ...
  if playerStatus == PROPOSED_STATUS then
    npcHandler:say(
      "Are you sure you want to remove your wedding proposal with {" .. getPlayerNameById(playerSpouse) .. "}?",
      cid
    )
    node:addChildKeyword(
      {"no"},
      StdModule.say,
      {npcHandler = npcHandler, onlyFocus = true, moveup = 3, text = "Ok, let's keep it then."}
    )
    ...
    node:addChildKeyword(
      ...
      {
        moveup = 3,
        text = "Ok, your marriage proposal to {" ..
          getPlayerNameById(playerSpouse) .. "} has been removed. Take your wedding ring back."
      }
    )
  else
...
local function confirmDivorce(cid, message, keywords, parameters, node)
  ...
  if playerStatus == MARRIED_STATUS then
    ...
    node:addChildKeyword(
      ...
      {
        moveup = 3,
        text = "Ok, you are now divorced of {" ..
          getPlayerNameById(playerSpouse) .. "}. Think better next time after marrying someone."
      }
    )
  else
...
node1:addChildKeyword(
  {"no"},
  StdModule.say,
  {npcHandler = npcHandler, onlyFocus = true, moveup = 1, text = "That's fine."}
)
local node2 =
  node1:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, onlyFocus = true, text = "And who would you like to marry?"}
)
...
local node3 =
  ...
  {"celebration"},
  StdModule.say,
  {npcHandler = npcHandler, onlyFocus = true, text = "Is your soulmate and friends here with you for the celebration?."}
)
node3:addChildKeyword(
  {"no"},
  StdModule.say,
  {npcHandler = npcHandler, onlyFocus = true, moveup = 1, text = "Then go bring them here!."}
)
local node4 =
  node3:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, onlyFocus = true, text = "Good, let's {begin} then!."}
) --, confirmWedding, {})
