...
local function greetCallback(cid)
  npcHandler:setMessage(
    MESSAGE_GREET,
    "Hello " ..
      (Player(cid):getSex() == PLAYERSEX_FEMALE and "beautiful lady" or "handsome gentleman") ..
...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "pumin") then
    if npcHandler.topic[cid] == 0 and player:getStorageValue(Storage.PitsOfInferno.Pumin) < 1 then
      npcHandler:say(
        "Sure, where else. Everyone likes to meet my master, he is a great demon, isn't he? Your name is ...?",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 3 then
      ...
      npcHandler:say(
        "How very interesting. I need to tell that to my master immediately. Please go to my colleagues and ask for Form 356. You will need it in order to proceed.",
        cid
      )
  ...
  elseif msgcontains(msg, "no") then
    if npcHandler.topic[cid] == 4 then
      ...
      npcHandler:say(
        "Oh, what a pity. Go see one of my colleagues. I give you the permission to get Form 287. Bye!",
        cid
      )
    end
