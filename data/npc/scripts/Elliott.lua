...
function creatureSayCallback(cid, type, msg)
  ...
  if (msgcontains(msg, "abandoned sewers")) then
    ...
    elseif (getPlayerStorageValue(cid, 10050) == 5) then
      npcHandler:say(
        "I'm glad to see you back alive and healthy. Did you find anything interesting that you want to {report}?",
        cid
      )
      npcHandler.topic[cid] = 7
    end
  elseif (msgcontains(msg, "mission")) then
    ...
    elseif (getPlayerStorageValue(cid, 10050) >= 3) then
      npcHandler:say(
        "Elliott's keeps calling it that. It's just another job! You fixed some broken pipes and stuff? Let me check, {ok}?",
        cid
      )
  ...
  elseif (msgcontains(msg, "yes")) then
    if (npcHandler.topic[cid] == 2) then
      npcHandler:say(
        "Good. Broken pipe and generator pieces, there's smoke evading. That's how you recognise them. See how you can fix them using your hands. Need about, oh, twenty of them at least repaired. Report to me or Jacob",
        cid
      )
