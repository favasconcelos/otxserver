...
local function creatureSayCallback(cid, type, msg)
  ...
  elseif
    (msgcontains("join", msg) or msgcontains("yes", msg)) and npcHandler.topic[cid] == 0 and
      player:getStorageValue(JOIN_STOR) ~= 1
   then
  ...
  elseif isInArray({"tasks", "task", "mission"}, msg:lower()) then
    ...
    if #can > 0 then
      ...
      npcHandler:say(
        "The current task" ..
          (#can > 1 and "s" or "") .. " that you can choose " .. (#can > 1 and "are" or "is") .. " " .. text,
        cid
      )
  ...
  elseif msg ~= "" and player:canStartTask(msg) then
    if #player:getStartedTasks() >= tasksByPlayer then
      npcHandler:say(
        "Sorry, but you already started " ..
          tasksByPlayer .. " tasks. You can check their {status}, {cancel} or {report} a task.",
        cid
      )
      return true
    end
    ...
    npcHandler:say(
      "In this task you must defeat " ..
        tasks[task].killsRequired .. " " .. tasks[task].raceName .. ". Are you sure that you want to start this task?",
      cid
    )
    choose[cid] = task
  ...
  elseif msg:lower() == "yes" and npcHandler.topic[cid] == 1 then
    ...
    npcHandler:say(
      "Excellent! You can check the {status} of your task saying {report} to me. Also you can {cancel} tasks to.",
      cid
    )
    choose[cid] = nil
    npcHandler.topic[cid] = 0
    ...
    if started and #started > 0 then
      ...
      for i = 1, #started do
        ...
        text =
          text ..
          "Task name: " ..
            tasks[id].raceName .. ". Current kills: " .. player:getStorageValue(KILLSSTORAGE_BASE + id) .. ".\n"
      end
    ...
    if started and #started > 0 then
      ...
      if not finishedAtLeastOne then
        ...
        if started and #started > 0 then
          ...
          npcHandler:say(
            "The current task" ..
              (#started > 1 and "s" or "") .. " that you started " .. (#started > 1 and "are" or "is") .. " " .. text,
            cid
          )
        end
      else
        npcHandler:say(
          "Awesome! you finished " ..
            (finished > 1 and "various" or "a") ..
              " task" .. (finished > 1 and "s" or "") .. ". Talk to me again if you want to start a {task}.",
          cid
        )
      end
    else
  ...
  elseif msg:lower() == "cancel" then
    ...
    if started and #started > 0 then
      npcHandler:say(
        "Cancelling a task will make the counter restart. Which of these tasks you want cancel?" ..
          (#started > 1 and "" or "") .. " " .. text,
        cid
      )
  ...
  elseif getTaskByName(msg) and npcHandler.topic[cid] == 2 and isInArray(getPlayerStartedTasks(cid), getTaskByName(msg)) then
    ...
    if player:getStorageValue(KILLSSTORAGE_BASE + task) > 0 then
      npcHandler:say(
        "You currently killed " ..
          player:getStorageValue(KILLSSTORAGE_BASE + task) ..
            "/" ..
              tasks[task].killsRequired ..
                " " ..
                  tasks[task].raceName ..
                    ". Cancelling this task will restart the count. Are you sure you want to cancel this task?",
        cid
      )
    else
  ...
  elseif msg:lower() == "yes" and npcHandler.topic[cid] == 3 then
    ...
    npcHandler:say(
      "You have cancelled the task " .. (tasks[cancel[cid]].name or tasks[cancel[cid]].raceName) .. ".",
      cid
    )
    npcHandler.topic[cid] = 0
  ...
  elseif isInArray({"special task"}, msg:lower()) then
    if player:getPawAndFurPoints() >= 70 then -- Tiquandas Revenge 70 points
      ...
      if player:getStorageValue(Storage.KillingInTheNameOf.MissionDemodras) == 1 then -- Check if he has already started the task.
        ...
        npcHandler:say(
          "This task is a very dangerous one. I want you to look for Demodras' hideout. It might be somewhere under the Plains of Havoc. Good luck, old chap.",
          cid
        )
      ...
      if
        (player:getStorageValue(Storage.KillingInTheNameOf.MissionDemodras) == 1 and
          ...
          player:getStorageValue(Storage.KillingInTheNameOf.TiquandasRevengeTeleport) == 0)
       then
