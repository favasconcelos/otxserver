...
local function creatureSayCallback(cid, type, msg)
  ...
  if (eventShopItems[npcHandler.topic[cid]]) then
    local itemId, itemCount, itemPrice =
      eventShopItems[npcHandler.topic[cid]][1],
      eventShopItems[npcHandler.topic[cid]][2],
      eventShopItems[npcHandler.topic[cid]][3]
    if (msg == "no" or msg == "n�o") then
      npcHandler:say("Ent�o qual item deseja comprar?", cid)
      npcHandler.topic[cid] = 0
    elseif (msg == "yes" or msg == "sim") then
      if (player:getItemCount(26143) > 0) then
        npcHandler:say(
          "Voc� comprou o Item {" ..
            npcHandler.topic[cid] .. "} " .. itemCount .. "x por " .. itemPrice .. " {Event Token(s)}!",
          cid
        )
...
npcHandler:setMessage(
  MESSAGE_GREET,
  "Ol�, |PLAYERNAME|! Caso n�o me conhe�a, v� no site e clique em {Event Shop}. Deseja trocar seus Event Tokens? fale {ofertas}."
)
