...
keywordHandler:addKeyword(
  {"job"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I run this armoury. If you want to protect your life, you better buy my wares."}
)
  ...
  if isInArray({"addon", "armor"}, msg) then
    if player:getStorageValue(Storage.OutfitQuest.WarriorShoulderAddon) == 5 then
      ...
      player:setStorageValue(
        Storage.OutfitQuest.WarriorShoulderTimer,
        os.time() + (player:getSex() == PLAYERSEX_FEMALE and 3600 or 7200)
      )
      npcHandler:say(
        "Ah, you must be the hero Trisha talked about. I'll prepare the shoulder spikes for you. Please give me some time to finish.",
        cid
      )
    elseif player:getStorageValue(Storage.OutfitQuest.WarriorShoulderAddon) == 6 then
      ...
      elseif
        player:getStorageValue(Storage.OutfitQuest.WarriorShoulderTimer) > 0 and
          player:getStorageValue(Storage.OutfitQuest.WarriorShoulderTimer) < os.time()
       then
        ...
        npcHandler:say(
          "I'm selling leather armor, chain armor, and brass armor. Ask me for a {trade} if you like to take a look.",
          cid
        )
      end
    end
