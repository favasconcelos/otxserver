  ...
  if isInArray({"sail", "passage", "wreck", "liberty bay", "ship"}, msg) then
    if player:getStorageValue(Storage.TheShatteredIsles.AccessToGoroma) ~= 1 then
      if player:getStorageValue(Storage.TheShatteredIsles.Shipwrecked) < 1 then
        npcHandler:say(
          "I'd love to bring you back to Liberty Bay, but as you can see, my ship is ruined. I also hurt my leg and can barely move. Can you help me?",
          cid
        )
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      if player:removeItem(5901, 30) then
        npcHandler:say(
          "Excellent! Now we can leave this godforsaken place. Thank you for your help. Should you ever want to return to this island, ask me for a passage to Goroma.",
          cid
        )
...
keywordHandler:addKeyword(
  {"job"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I'm the captain of this - well, wreck. Argh."}
)
keywordHandler:addKeyword(
  {"captain"},
  StdModule.say,
