...
local config = {
  ...
  ["spools of yarn"] = {
    storageValue = 7,
    text = {
      "Brought the 10 spools of yarn?",
      "Thanks. That's it, you're done. Good job, |PLAYERNAME|. I keep my promise. Here's my old assassin head piece."
    },
    itemId = 5886,
    count = 10
  }
...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "addon") then
    if
      player:hasOutfit(player:getSex() == PLAYERSEX_FEMALE and 156 or 152) and
        player:getStorageValue(Storage.OutfitQuest.AssassinFirstAddon) < 1
     then
      npcHandler:say(
        "Vescu gave you an assassin outfit? Haha. Noticed it lacks the head piece? You look a bit silly. Want my old head piece?",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      ...
      player:setStorageValue(
        Storage.OutfitQuest.AssassinFirstAddon,
        player:getStorageValue(Storage.OutfitQuest.AssassinFirstAddon) + 1
      )
