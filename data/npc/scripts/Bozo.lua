...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "join") then
    if player:getStorageValue(Storage.WhatAFoolishQuest.Questline) ~= -1 then
      npcHandler:say(
        "Wow, your stupidity would be pride and joy for every fool. You've already applied as a member. Let's rather talk about your current mission.",
        cid
      )
      return true
    end
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      ...
      if targetValue.removeItem then
        if
          not player:removeItem(
            targetValue.removeItem.itemId,
            targetValue.removeItem.count or 1,
            targetValue.removeItem.subType or -1
          )
         then
          npcHandler:say(targetValue.text[2], cid)
          npcHandler.topic[cid] = 0
      ...
      if targetValue.pie then
        if
          player:getStorageValue(Storage.WhatAFoolishQuest.PieBoxTimer) > 0 and
            player:getStorageValue(Storage.WhatAFoolishQuest.PieBoxTimer) < os.time()
         then
          npcHandler:say(
            "Eeeek! What have you done?? These pies are crawling with bugs! Those must be the infamous parcel bugs! Get some new pies at once you wannabe fool, and this time without any bugs!",
            cid
          )
          npcHandler.topic[cid] = 0
          return true
  ...
  elseif msgcontains(msg, "no") and npcHandler.topic[cid] ~= 0 then
    ...
    elseif isInArray({3, 4}, npcHandler.topic[cid]) then
      if
        player:getStorageValue(Storage.WhatAFoolishQuest.Questline) == 11 and
          player:getStorageValue(Storage.WhatAFoolishQuest.EmperorBeardShave) == 1
       then
      ...
      elseif
        player:getStorageValue(Storage.WhatAFoolishQuest.Questline) == 35 and
          player:getStorageValue(Storage.WhatAFoolishQuest.LostDisguise) ~= 1
       then
        player:addItem(7502, 1)
        player:setStorageValue(Storage.WhatAFoolishQuest.LostDisguise, 1)
        npcHandler:say(
          "You wasted the disguise?? Why do only fools apply for the fools guild? Here... try again, but be wittier this time.",
          cid
        )
      else
...
keywordHandler:addKeyword(
  {"sorcerer"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I wanted to become a sorcerer, too, but I was overqualified!"},
  function(player)
...
keywordHandler:addKeyword(
  {"druid"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I wanted to become a druid, too, but I was overqualified!"},
  function(player)
...
keywordHandler:addKeyword(
  {"paladin"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I wanted to become a paladin, too, but I was overqualified!"},
  function(player)
...
keywordHandler:addKeyword(
  {"knight"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I wanted to become a knight, too, but I was overqualified!"},
  function(player)
...
keywordHandler:addKeyword(
  {"sorcerer"},
  StdModule.say,
  {npcHandler = npcHandler, text = "The good thing about them is that they can't be at two places at the same time."}
)
keywordHandler:addKeyword(
  {"druid"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I wonder if they love my water squirt flowers as much as all other plants."}
)
keywordHandler:addKeyword(
  {"paladin"},
  StdModule.say,
  {npcHandler = npcHandler, text = "They are the king's favourites, because they know how to 'bow'."}
)
keywordHandler:addKeyword(
  {"knight"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Did you notice that old knights have their scars just on their backs?"}
)

keywordHandler:addKeyword(
  {"here"},
  StdModule.say,
  {npcHandler = npcHandler, text = "A fitting place for a {jester}. I guess there are worse {jobs} around."}
)
keywordHandler:addKeyword(
  {"king"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Bozo: Nah, no jests about His Royal Highness."}
)
keywordHandler:addKeyword(
  {"tibia"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I rarely leave the castle. It's really stressful to be as popular as me."}
)
keywordHandler:addKeyword(
  {"castle"},
  StdModule.say,
  {
    ...
    text = "This castle is my home. A fitting place for a jester and all other fools. Feel welcome."
  }
)
...
keywordHandler:addKeyword(
  {"name"},
  StdModule.say,
  {npcHandler = npcHandler, text = "My name is Bozo. But it's more than a name, it's a lifestyle for me!"}
)
...
keywordHandler:addKeyword(
  ...
  {
    npcHandler = npcHandler,
    text = "Ever since the first guild was created, there is a great demand of jesters and fools to join them."
  }
)
keywordHandler:addKeyword(
  {"sell"},
  ...
  {
    npcHandler = npcHandler,
    text = "Sell? Hmm, I know a little about magic and by chance I can sell you a truly unusual {weapon}."
  }
)
...
keywordHandler:addKeyword(
  {"news"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I know the newest jokes in tibia."}
)
keywordHandler:addKeyword(
  {"how", "are", "you"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Thank you, I'm fine, the gods are with me."}
)
keywordHandler:addKeyword(
  {"necromant", "nectar"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Pheeew! That sounds disgusting! Are you a cook at Frodo's?"}
)
...
keywordHandler:addKeyword(
  {"noodles"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Hey, the little one is almost as funny as me!"}
)
keywordHandler:addKeyword(
  {"muriel"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Better don't mess with sorcerers!"}
)
keywordHandler:addKeyword(
  {"elane"},
  StdModule.say,
  {npcHandler = npcHandler, text = "She's pretty but too serious for my taste."}
)
keywordHandler:addKeyword(
  {"marvik"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "Humourless old guy! Once, he turned me into a frog for painting his distasteful cave in pink."
  }
)
keywordHandler:addKeyword(
  {"gregor"},
  StdModule.say,
  {npcHandler = npcHandler, text = "A man of steel with a stomach of wax. Never offer him a beer!"}
)
keywordHandler:addKeyword(
  {"quentin"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "He's my baby brother. If you tell him I sent you, he will grant you an extra spell or two."
  }
)
keywordHandler:addKeyword(
  {"gorn"},
  ...
  {
    npcHandler = npcHandler,
    text = "He sells spell scrolls each day at midnight, but you have to address him that very second."
  }
)
keywordHandler:addKeyword(
  {"god"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I better make no jokes about THIS matter."}
)
keywordHandler:addKeyword(
  {"sam"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "Did you know that he sells a 'power axe of doom' now? Run and buy it, he only has got three in store."
  }
)
keywordHandler:addKeyword(
  {"benjamin"},
  StdModule.say,
  {npcHandler = npcHandler, text = "He would make a fine jester, too."}
)
keywordHandler:addKeyword(
  {"monster"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I know a lot of monster jokes. Just tell me a monster's name, come on."}
)
keywordHandler:addKeyword(
  {"demon"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Why are the experienced heroes quicker than others? ... The demons love fast food!"}
)
keywordHandler:addKeyword(
  {"ghoul"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Where do ghouls buy their robes? ... In a boooohtique!"}
)
keywordHandler:addKeyword(
  {"dragon"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Why do dragons breathe fire? ... They ate too many sorcerers in chilli sauce!"}
)
keywordHandler:addKeyword(
  {"orc"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Why do orcs have green skin? ... They ate at Frodo's!"}
)
keywordHandler:addKeyword(
  {"cyclops"},
  StdModule.say,
  {npcHandler = npcHandler, text = "How many eyes does a cyclops have? ... One for each IQ point they have!"}
)
keywordHandler:addKeyword(
  {"oswald"},
  StdModule.say,
  {npcHandler = npcHandler, text = "If you believe half the rumours he's spreading, you will get in a lot of trouble."}
)
keywordHandler:addKeyword(
  {"dungeon"},
  StdModule.say,
  {npcHandler = npcHandler, text = "If you are a bad jester, you get a chance to visit them now and then."}
)
keywordHandler:addKeyword(
  {"mino"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "What do all little minotaurs want to become when they are grown-ups? ... Cowboys, of course!"
  }
)
keywordHandler:addKeyword(
  {"troll"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Why do trolls live underground? ... Because there are so many pks on the surface!"}
)
keywordHandler:addKeyword(
  {"bonelord"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Why are bonelords so ugly? ... Because their mom and dad were bonelords, too!"}
)
keywordHandler:addKeyword(
  {"rat"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Why does the rat have a wooden leg? ... Because it is a former pirate!"}
)
keywordHandler:addKeyword(
  {"spider"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "Why did the spider cross the road? ... Because it ... oh you already know this one!?"
  }
)
...
keywordHandler:addKeyword(
  {"durin"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Isn't he the author of the book 'Fun with Demons'?"}
)
keywordHandler:addKeyword(
  {"stephan"},
  StdModule.say,
  {npcHandler = npcHandler, text = "He is kind of a father figure to me. Of course he denies all kinship to me."}
)
keywordHandler:addKeyword(
  {"steve"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "He's a smart one. I heared he hid in a foreign country as the first bugs showed up."
  }
)
keywordHandler:addKeyword(
  {"excalibug"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I am not foolish enough to believe in the existence of this weapon."}
)
keywordHandler:addKeyword(
  {"wall", "carving"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "Oh, I saw some demon carvings in the dungeons as I hid there after a little joke on old Stutch."
  }
)
keywordHandler:addKeyword(
  {"demon", "carving"},
  ...
  {
    npcHandler = npcHandler,
    text = "Yes, they showed demons, seven actually, dancing around a sword! In something like a flaming pit."
  }
)
keywordHandler:addKeyword(
  {"flaming", "pit"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Ah, don't ask me! Usually mages and mystics know more about such stuff."}
)
...
local jobKeyword =
  ...
  {
    npcHandler = npcHandler,
    text = "I'm the royal jes ... uhm ... the royal tax-collector! Do you want to pay your taxes?"
  }
)
jobKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Come back, when you have enough money.", reset = true},
  function(player)
...
jobKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Thank you very much. I will have a drink or two on your health!", reset = true},
  nil,
  function(player)
...
local magicKeyword =
  ...
  {
    npcHandler = npcHandler,
    text = "I actually know some spells! Do you want to learn how to 'lessen your load' for 200 gold?"
  }
)
magicKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Come back, when you have enough money.", reset = true},
  function(player)
...
magicKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Here you are, I already lessened your load.", reset = true},
  nil,
  function(player)
...
magicKeyword:addChildKeyword(
  {""},
  StdModule.say,
  {npcHandler = npcHandler, text = "You don't know what offer you are missing!", reset = true}
)
...
local weaponKeyword =
  ...
  {"weapon"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Do you want to buy a 'mace of the fury' for 250 gold?"}
)
weaponKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Come back, when you have enough money.", reset = true},
  function(player)
...
weaponKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "And here it is, it suits you well!", reset = true},
  nil,
  function(player)
...
weaponKeyword:addChildKeyword(
  {""},
  StdModule.say,
  {npcHandler = npcHandler, text = "You dont know what offer you have passed!", reset = true}
)

keywordHandler:addKeyword(
  {"kiss"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Uh, go away!", ungreet = true},
  function(player)
...
local kissKeyword =
  keywordHandler:addKeyword({"kiss"}, StdModule.say, {npcHandler = npcHandler, text = "Do you want to kiss me?"})
kissKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Uh, oh! ... I am seeing stars!", reset = true},
  nil,
  function(player)
    player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
  end
...
kissKeyword:addChildKeyword(
  {""},
  StdModule.say,
  {npcHandler = npcHandler, text = "Pah, I didn't want to kiss you anyway!", reset = true}
)
...
keywordHandler:addKeyword(
  ...
  {
    npcHandler = npcHandler,
    text = "Well, women don't behave necessarily in a ladylike way just because they dress like one!"
  },
  function(player)
    return player:getSex() == PLAYERSEX_MALE
  end
...
local ladyKeyword =
  ...
  {"lady"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Has any man said to you that you're not only beautiful but also intelligent?"}
)
ladyKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "This is a world of fantasy and full of surprises!", reset = true}
)
ladyKeyword:addChildKeyword(
  {""},
  StdModule.say,
  {npcHandler = npcHandler, text = "Well, think about it!", reset = true}
)
