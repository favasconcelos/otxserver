...
local function creatureSayCallback(cid, type, msg)
  ...
  if (msgcontains(msg, "mission")) then
    --  Matchmaker
    if
      player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 16 and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) < 17
     then
      npcHandler:say(
        "For your rank there are two missions available: {matchmaker} and {golem repair}. You can undertake each mission, but you can turn in a specific mission only once every 20 hours. ",
        cid
      )
      npcHandler.topic[cid] = 0
    elseif player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 17 then
      npcHandler:say(
        "For your rank there are four missions available: {matchmaker}, {golem repair}, {spore gathering} and {grindstone hunt}. You can undertake each mission, but you can turn in a specific mission only once every 20 hours.",
        cid
      )
      npcHandler.topic[cid] = 0
    end
  elseif (msgcontains(msg, "matchmaker")) then
    ...
    -- Golem Repair
    if
      player:getStorageValue(Storage.BigfootBurden.MissionMatchmaker) < 1 and
        player:getStorageValue(Storage.BigfootBurden.MatchmakerTimeout) < os.time() and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 16
     then
      npcHandler:say(
        {
    ...
    elseif (npcHandler.topic[cid] == 1 or npcHandler.topic[cid] == 2) then
      if player:getStorageValue(Storage.BigfootBurden.MatchmakerStatus) == 1 then
        ...
        npcHandler:say(
          "Gnomo arigato |PLAYERNAME|! You did well. That will help us a lot. Take your tokens and this gnomish supply package as a reward. ",
          cid
        )
        npcHandler.topic[cid] = 0
      end
  ...
  elseif (msgcontains(msg, "repair")) then
    ...
    -- Spore Gathering
    if
      player:getStorageValue(Storage.BigfootBurden.MissionTinkersBell) < 1 and
        player:getStorageValue(Storage.BigfootBurden.TinkerBellTimeout) < os.time() and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 16
     then
      npcHandler:say(
  ...
  elseif (msgcontains(msg, "spore")) then
    ...
    -- Grindstone Hunt
    if
      player:getStorageValue(Storage.BigfootBurden.MissionSporeGathering) < 1 and
        player:getStorageValue(Storage.BigfootBurden.SporeGatheringTimeout) < os.time() and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 17
     then
      npcHandler:say(
        {
  ...
  elseif (msgcontains(msg, "grindstone")) then
    -- Grindstone Hunt
    if
      player:getStorageValue(Storage.BigfootBurden.MissionGrindstoneHunt) < 1 and
        player:getStorageValue(Storage.BigfootBurden.GrindstoneTimeout) < os.time() and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 17
     then
      npcHandler:say(
        {
  ...
  elseif (msgcontains(msg, "report")) then
    ...
    elseif player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 17 then
      npcHandler:say(
        "Which mission do you want to report: {matchmaker}, {golem repair}, {spore gathering} or {grindstone hunt}?",
        cid
      )
