...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "adventures") or msgcontains(msg, "join") then
    if Player(cid):getStorageValue(Storage.BigfootBurden.QuestLine) < 1 then
      ...
      npcHandler:say("You already talked with me.", cid)
    end
  elseif msgcontains(msg, "recruiting") then
    if npcHandler.topic[cid] == 1 then
      npcHandler:say(
        "Ok, so listen. Your help is needed. That is if you're the hero type. Our ... {partners} need some help in urgent matters.",
        cid
      )
  ...
  elseif msgcontains(msg, "partners") then
    if npcHandler.topic[cid] == 2 then
      npcHandler:say(
        "I guess the time of secrecy is over now. Well, we have an old alliance with another underground dwelling race, the {gnomes}.",
        cid
      )
