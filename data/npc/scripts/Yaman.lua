...
local function creatureSayCallback(cid, type, msg)
  ...
  elseif isInArray({"job", "items", "Items", "Job"}, msg) then
    npcHandler:say(
      "I trade Enchanted Chicken Wing for Boots of Haste, Warrior Sweat for 4 Warrior Helmets, Fighting Spirit for 2 Royal Helmet Magic Sulphur for 3 Fire Swords",
      cid
    )
  ...
  elseif msgcontains(msg, "cookie") then
    if
      player:getStorageValue(Storage.WhatAFoolishQuest.Questline) == 31 and
        player:getStorageValue(Storage.WhatAFoolishQuest.CookieDelivery.Djinn) ~= 1
     then
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 5 then
      ...
      npcHandler:say(
        "You see, good deeds like this will ... YOU ... YOU SPAWN OF EVIL! I WILL MAKE SURE THE MASTER LEARNS ABOUT THIS!",
        cid
      )
