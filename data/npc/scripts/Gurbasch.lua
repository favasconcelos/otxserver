...
local function addTravelKeyword(keyword, text, cost, discount, destination)
  local travelKeyword =
    ...
    {keyword},
    StdModule.say,
    {npcHandler = npcHandler, text = text[1], cost = cost, discount = discount}
  )
...
addTravelKeyword(
  "kazordoon",
  {"Do you want to go to Kazordoon to try the beer there? |TRAVELCOST|?", "Set the sails!", "Then not."},
  160,
  "postman",
  Position(32660, 31957, 15)
)

keywordHandler:addKeyword(
  {"passage"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Do you want me take you to {Kazordoon} or {Farmine}?"}
)
