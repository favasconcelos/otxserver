...
local function creatureSayCallback(cid, type, msg)
  ...
  if (msgcontains(msg, "mission")) then
    -- Crystal Keeper
    if
      player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 14 and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) < 15
     then
  ...
  elseif (msgcontains(msg, "keeper")) then
    ...
    if
      player:getStorageValue(Storage.BigfootBurden.MissionCrystalKeeper) < 1 and
        player:getStorageValue(Storage.BigfootBurden.CrystalKeeperTimout) < os.time()
     then
      npcHandler:say(
        "You will have to repair some damaged crystals. Go into the Crystal grounds and repair them, using this harmonic crystal. Repair five of them and return to me. ",
        cid
      )
      npcHandler:say("If you lose a mission item you can probably buy it from Gnomally. ", cid)
    ...
    elseif (npcHandler.topic[cid] == 1 or npcHandler.topic[cid] == 2) then
      if player:getStorageValue(Storage.BigfootBurden.RepairedCrystalCount) == 5 and player:removeItem(18219, 1) then
        ...
        npcHandler:say(
          "You did well. That will help us a lot. Take your token and this gnomish supply package as a reward. ",
          cid
        )
        npcHandler.topic[cid] = 0
      else
        npcHandler:say("You did not repaired enough crystal or you have not asked for this task.", cid)
      end
    else
      npcHandler:say(
        "You have already completed/asked/still in that mission or just try again in a few hours or your rank is not suitable for this mission.",
        cid
      )
    end
  elseif (msgcontains(msg, "spark")) then
    ...
    -- Exterminators
    if
      player:getStorageValue(Storage.BigfootBurden.MissionRaidersOfTheLostSpark) < 1 and
        player:getStorageValue(Storage.BigfootBurden.RaidersOfTheLostSparkTimeout) < os.time()
     then
      npcHandler:say(
        "Take this extractor and drive it into a body of a slain crystal crusher. This will charge your own body with energy sparks. Charge it with seven sparks and return to me. ...",
        cid
      )
      npcHandler:say(
        "Don't worry. The gnomes assured me you'd be save. That is if nothing strange or unusual occurs! ",
        cid
      )
    ...
    elseif (npcHandler.topic[cid] == 1 or npcHandler.topic[cid] == 2) then
      if player:getStorageValue(Storage.BigfootBurden.ExtractedCount) == 7 then
        ...
        npcHandler:say("You did not draw enough energy from Crystal Crushers or you have not asked for this task.", cid)
      end
    else
      npcHandler:say(
        "You have already completed/asked/still in that mission or just try again in a few hours or your rank is not suitable for this mission.",
  ...
  elseif (msgcontains(msg, "extermination")) then
    ...
    -- Mushroom Digger
    if
      player:getStorageValue(Storage.BigfootBurden.MissionExterminators) < 1 and
        player:getStorageValue(Storage.BigfootBurden.ExterminatorsTimeout) < os.time() and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 15
     then
      npcHandler:say(
        "The wigglers have become a pest that threaten our resources and supplies. Kill 10 wigglers in the caves like the mushroon gardens or the truffles ground. Report back to me when you are done. ",
        cid
      )
    ...
    elseif (npcHandler.topic[cid] == 2) then
      if player:getStorageValue(Storage.BigfootBurden.ExterminatedCount) == 10 then
        ...
        npcHandler:say("You did not killed enough wigglers or you have not asked for this task.", cid)
      end
    else
      npcHandler:say(
        "You have already completed/asked/still in that mission or just try again in a few hours or your rank is not suitable for this mission.",
  ...
  elseif (msgcontains(msg, "digging")) then
    -- Mushroom Digger
    if
      player:getStorageValue(Storage.BigfootBurden.MissionMushroomDigger) < 1 and
        player:getStorageValue(Storage.BigfootBurden.MushroomDiggerTimeout) < os.time() and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 15
     then
      npcHandler:say(
        {
    ...
    elseif (npcHandler.topic[cid] == 2) then
      if player:getStorageValue(Storage.BigfootBurden.MushroomCount) == 3 and player:removeItem(18339, 1) then
        ...
        npcHandler:say("You did not completed this task yet or you have not asked for this task.", cid)
      end
    else
      npcHandler:say(
        "You have already completed/asked/still in that mission or just try again in a few hours or your rank is not suitable for this mission.",
  ...
  elseif (msgcontains(msg, "report")) then
    ...
    elseif player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 15 then
      npcHandler:say(
        "Which mission do you want to report: {crystal keeper}, {spark hunting}, {extermination} or {mushroom digging}?",
        cid
      )
