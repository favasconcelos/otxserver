...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "join") then
    if
      player:getStorageValue(Storage.OutfitQuest.BrotherhoodOutfit) < 1 and
        player:getStorageValue(Storage.OutfitQuest.NightmareOutfit) < 1
     then
      npcHandler:say(
        {
  ...
  elseif msgcontains(msg, "advancement") then
    ...
    elseif player:getStorageValue(Storage.OutfitQuest.NightmareOutfit) == 3 then
      npcHandler:say(
        "So you want to advance to a {Lord Protector} rank? Did you bring 1500 demonic essences with you?",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") then
    if npcHandler.topic[cid] == 1 then
      npcHandler:say(
        "Please know that your decision is irrevocable. You will abandon the opportunity to join any order whose doctrine is incontrast to our own ...",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 3 then
      if player:removeItem(6500, 500) then
        ...
        npcHandler:say(
          "You advanced to {Initiate} rank! You are now able to use teleports of second floor of Knightwatch Tower.",
          cid
        )
      else
    ...
    elseif npcHandler.topic[cid] == 5 then
      if player:removeItem(6500, 1500) then
        ...
        npcHandler:say(
          "You advanced to {Lord Protector} rank! You are now able to use teleports of fourth floor of Knightwatch Tower and to create addon scrolls.",
          cid
        )
      else
        npcHandler:say("Come back when you gather all essences.", cid)
