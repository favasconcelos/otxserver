...
local function creatureSayCallback(cid, type, msg)
  ...
  if
    msgcontains(msg, "addon") or msgcontains(msg, "outfit") or (addonProgress == 1 and msgcontains(msg, "leather")) or
      ((addonProgress == 1 or addonProgress == 2) and msgcontains(msg, "backpack"))
   then
    ...
    elseif addonProgress == 1 then
      npcHandler:say(
        "Ah, right, almost forgot about the backpack! Have you brought me 100 pieces of minotaur leather as requested?",
        cid
      )
    ...
    elseif addonProgress == 2 then
      if player:getStorageValue(Storage.OutfitQuest.Citizen.AddonBackpackTimer) < os.time() then
        ...
        player:setStorageValue(
          Storage.OutfitQuest.Ref,
          math.min(0, player:getStorageValue(Storage.OutfitQuest.Ref) - 1)
        )
        ...
        npcHandler:say(
          "Uh... I didn't expect you to return that early. Sorry, but I'm not finished yet with your backpack. I'm doing the best I can, promised.",
          cid
        )
      end
    elseif addonProgress == 3 then
      npcHandler:say(
        "Sorry, but I can only make one backpack per person, else I'd have to close my shop and open a leather manufactory.",
        cid
      )
    end
    return true
  ...
  if npcHandler.topic[cid] == 1 then
    if msgcontains(msg, "backpack") or msgcontains(msg, "minotaur") or msgcontains(msg, "leather") then
      npcHandler:say(
        "Well, if you really like this backpack, I could make one for you, but minotaur leather is hard to come by these days. Are you willing to put some work into this?",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 2 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Alright then, if you bring me 100 pieces of fine minotaur leather I will see what I can do for you. You probably have to kill really many minotaurs though... so good luck!",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 3 then
    if msgcontains(msg, "yes") then
      if player:getItemCount(5878) < 100 then
        npcHandler:say(
          "Sorry, but that's not enough leather yet to make one of these backpacks. Would you rather like to buy a normal backpack for 10 gold?",
          cid
        )
      else
      ...
      npcHandler:say(
        "I know, it's quite some work... don't lose heart, just keep killing minotaurs and you'll eventually get lucky. Would you rather like to buy a normal backpack for 10 gold?",
        cid
      )
    end
    npcHandler.topic[cid] = 0
...
keywordHandler:addKeyword(
  {"how", "are", "you"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I'm recovering from a {sea} passage."}
)
keywordHandler:addKeyword(
  {"job"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I'm an {explorer} who seeks {adventures}."}
)
keywordHandler:addKeyword(
  {"explore"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I've been almost everywhere in {Tibia}."}
)
keywordHandler:addKeyword(
  {"adventure"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"time"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Sorry, I lost my watch in a storm."}
)
keywordHandler:addKeyword(
  {"help"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"seymour"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I think this poor guy was a bad choice as head of the {academy}."}
)
keywordHandler:addKeyword(
  {"academy"},
  StdModule.say,
  {npcHandler = npcHandler, text = "A fine institution, but it needs definitely more funds from the {king}."}
)
keywordHandler:addKeyword(
  {"king"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"thais"},
  StdModule.say,
  {npcHandler = npcHandler, text = "A fine city, but the {king} has some problems enforcing the law."}
)
keywordHandler:addKeyword(
  {"weapon"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"magic"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Spells are only taught in the guildhalls of the mainland."}
)
keywordHandler:addKeyword(
  {"tibia"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I'm trying to explore each spot of Tibia, and one day I'll have seen it all."}
)
keywordHandler:addKeyword(
  {"castle"},
  StdModule.say,
  {npcHandler = npcHandler, text = "If you travel to Thais, you really should visit the marvellous castle there."}
)
keywordHandler:addKeyword(
  {"mainland"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"tools"},
  StdModule.say,
  {npcHandler = npcHandler, text = "The most important tools you need are a {rope}, a {shovel} and maybe a {torch}."}
)
keywordHandler:addKeyword(
  {"rope"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"shovel"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Sometimes I have the strong urge to use it to knock someone out."}
)
keywordHandler:addKeyword(
  {"torch"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"bank"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I have almost no money with me anyway, so no need to deposit any at the bank."}
)
keywordHandler:addKeyword(
  {"destiny"},
  StdModule.say,
  {npcHandler = npcHandler, text = "You will find your destiny. I'm sure it's something big and important."}
)
keywordHandler:addKeyword(
  {"academy"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"trade"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Oh, I'm sorry, but I'm not interested in buying or selling anything."}
)
keywordHandler:addKeyword(
  {"premium"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I wouldn't want to miss being a premium adventurer. Everything is so much easier!"}
)
...
keywordHandler:addKeyword(
  {"al", "dee"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I don't have much to say about him. I think he sells {tools}."}
)
...
keywordHandler:addKeyword(
  {"santiago"},
  StdModule.say,
  {npcHandler = npcHandler, text = "He promised to repair my {raft}."}
)
keywordHandler:addKeyword(
  {"amber"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Did you know my name is also the name of a gem?"}
)
keywordHandler:addKeyword(
  {"tom"},
  StdModule.say,
  {npcHandler = npcHandler, text = "To me he seems a bit rude, but maybe that's just my impression."}
)
keywordHandler:addKeyword(
  {"lee'delle"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I heard her offers are extraordinarily good."}
)
keywordHandler:addKeyword(
  {"oracle"},
  StdModule.say,
  {npcHandler = npcHandler, text = "The oracle is said to show you your {destiny} once you are level 8."}
)
keywordHandler:addKeyword(
  {"norma"},
  StdModule.say,
  {npcHandler = npcHandler, text = "She has changed a lot since I last saw her."}
)
keywordHandler:addKeyword(
  {"seymour"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"lily"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Hm, I think I haven't met her yet."}
)
keywordHandler:addKeyword(
  {"billy"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"paulie"},
  StdModule.say,
  {npcHandler = npcHandler, text = "No, I didn't go to the {bank} yet."}
)
keywordHandler:addKeyword(
  {"cipfried"},
  StdModule.say,
  {npcHandler = npcHandler, text = "A gentle person. You should visit him if you have questions or need healing."}
)
keywordHandler:addKeyword(
  {"hyacinth"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Hyacinth is a great healer. He lives somewhere hidden on this isle."}
)
keywordHandler:addKeyword({"obi"}, StdModule.say, {npcHandler = npcHandler, text = "He's a funny little man."})
keywordHandler:addKeyword(
  {"dixi"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I don't really know her, but she seems to be a nice girl."}
)
keywordHandler:addKeyword(
  {"zerbrus"},
  StdModule.say,
  {npcHandler = npcHandler, text = "An extraordinary warrior. He's the first and last line of defence of {Rookgaard}."}
)
...
prisonerKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "It's 'mok' in Orcish. I tell you more if you bring me some {food}.", reset = true}
)
prisonerKeyword:addChildKeyword(
  {"no"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"food"},
  StdModule.say,
  {npcHandler = npcHandler, text = "My favourite dish is {salmon}. Oh please, bring me some."}
)
local salmonKeyword =
  keywordHandler:addKeyword(
  {"salmon"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Yeah! If you give me some salmon, I'll tell you another Orcish word. Okay?"}
)
salmonKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Thank you. Orcs call arrows 'pixo'.", reset = true},
  function(player)
...
salmonKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "You don't have any salmon!", reset = true}
)
salmonKeyword:addChildKeyword(
  {""},
  StdModule.say,
  {npcHandler = npcHandler, text = "Ok, then I won't tell you another Orcish word.", reset = true}
)

-- Logbook Quest
local bookKeyword =
  keywordHandler:addKeyword({"book"}, StdModule.say, {npcHandler = npcHandler, text = "Do you bring me my notebook?"})
bookKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Excellent. Here, take this short sword as a reward.", reset = true},
  function(player)
    return player:getItemCount(1955) > 0
  end,
  function(player)
    player:addItem(2406, 1)
    player:removeItem(1955, 1)
  end
)
bookKeyword:addChildKeyword(
  {"yes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Mhm, whatever you have there, it is not my notebook.", reset = true}
)
...
npcHandler:setMessage(
  MESSAGE_GREET,
  "Oh hello, nice to see you |PLAYERNAME|. Are you here to hear some stories of my {adventures} or do you need {help}?"
)
