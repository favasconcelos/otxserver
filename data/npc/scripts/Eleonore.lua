...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "ring") or msgcontains(msg, "mission") then
    ...
    elseif
      player:getStorageValue(Storage.TheShatteredIsles.TheGovernorDaughter) == 3 and
        player:getStorageValue(Storage.TheShatteredIsles.TheErrand) < 1
     then
  ...
  elseif msgcontains(msg, "errand") then
    if player:getStorageValue(Storage.TheShatteredIsles.TheErrand) == 2 then
      npcHandler:say(
        "Great, thank you! As promised, here are your 5 gold pieces. Is there ... anything left that you might want to discuss with me?",
        cid
      )
      player:addMoney(5)
  ...
  elseif msgcontains(msg, "peg leg") then
    if player:getStorageValue(Storage.TheShatteredIsles.TheErrand) == 3 then
      npcHandler:say(
        "You have returned my ring and proven yourself as trustworthy. There is something I have to discuss with you. Are you willing to listen?",
        cid
      )
  ...
  elseif msgcontains(msg, "mermaid") then
    if player:getStorageValue(Storage.TheShatteredIsles.AccessToLagunaIsland) == 1 then
      npcHandler:say(
        "I can't thank you enough for freeing my beloved Ray from that evil spell. I am still shocked that a mermaid could steal his love that easily.",
        cid
      )
      npcHandler.topic[cid] = 0
    end
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 2 then
      npcHandler:say(
        "Thank you! It is not a difficult matter but a rather urgent one. I need to send some money to a person in town. Would you be willing to run this small errand for me?",
        cid
      )
