...
function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "gnome") then
    return npcHandler:say(
      "Gnomes have lived autonomous for so long that it still feels odd to work with strangers for many of us.",
      cid
    )
  end
  ...
  if msgcontains(msg, "mission") then
    if player:getLevel() > levels[2] then
      ...
      npcHandler:say(
        " I can offer you several missions: to gather geomantic {charges}, to {fertilise} the mushroom caves, to destroy monster {nests} and to {kill} some crystal crushers.",
        cid
      )
    end
    return
  ...
  if msgcontains(msg, "report") then
    ...
    return npcHandler:say(
      "What mission do you want to report about: gathering the geomantic {charges}, the {fertilisation} of the mushroom caves, about destroying monster {nests} and the {killing} of crystal crushers?",
      cid
    )
  end
  ...
  if talkState[cid] == "report" then
    if msgcontains(msg, "charges") then
      ...
      elseif player:getStorageValue(SPIKE_MIDDLE_CHARGE_MAIN) == 2 then
        ...
        npcHandler:say(
          "Gnowful! Charge this magnet at three monoliths in the cave system. With three charges, the magnet will disintegrate and charge you with its gathered energies. Step on the magnetic extractor here to deliver the charge to us, then report to me.",
          cid
        )
      end
  ...
  if msgcontains(msg, "charges") then
    if player:getExhaustion(SPIKE_MIDDLE_CHARGE_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_MIDDLE_CHARGE_DAILY) - os.time()) ..
    ...
    if player:getStorageValue(SPIKE_MIDDLE_CHARGE_MAIN) == -1 then
      npcHandler:say(
        {
          "Our mission for you is to use a magnet on three different monoliths in the cave system here. After the magnet evaporates on the last charge, enter the magnetic extractor here to deliver your charge.",
  ...
  if msgcontains(msg, "fertilise") then
    if player:getExhaustion(SPIKE_MIDDLE_MUSHROOM_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_MIDDLE_MUSHROOM_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_MIDDLE_MUSHROOM_MAIN) == -1 then
      npcHandler:say(
        "Your mission would be to seek out gardener mushrooms in the caves and use some fertiliser on them. If you are interested, I can give you some more information about it. Are you willing to accept this mission?",
        cid
      )
  ...
  if talkState[cid] == "fertilise" then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Gnometastic! And here is your fertiliser - use it on four gardener mushroom in the caves. If you lose the fertiliser you'll have to bring your own. Gnomux sells all the equipment that is required for our missions.",
        cid
      )
      talkState[cid] = nil
    elseif msgcontains(msg, "no") then
  ...
  if msgcontains(msg, "nests") then
    if player:getExhaustion(SPIKE_MIDDLE_NEST_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_MIDDLE_NEST_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_MIDDLE_NEST_MAIN) == -1 then
      npcHandler:say(
        "Our mission for you is to step into the gnomish transformer and then destroy eight monster nests in the caves. If you are interested, I can give you some more information about it. Are you willing to accept this mission?",
        cid
      )
      talkState[cid] = "nests"
    else
  ...
  if talkState[cid] == "nests" then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Gnometastic! Don't forget to step into the transformer before you go out and destroy five monster nests. If your transformation runs out, return to the transformer to get another illusion.",
        cid
      )
      talkState[cid] = nil
    elseif msgcontains(msg, "no") then
  ...
  if msgcontains(msg, "kill") then
    if player:getExhaustion(SPIKE_MIDDLE_KILL_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_MIDDLE_KILL_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_MIDDLE_KILL_MAIN) == -1 then
      npcHandler:say(
        "This mission will require you to kill some crystal crushers for us. If you are interested, I can give you some more information about it. Are you willing to accept this mission?",
        cid
      )
      talkState[cid] = "kill"
    else
  ...
  if talkState[cid] == "kill" then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Gnometastic! You should have no trouble to find enough crystal crushers. Killing seven of them should be enough.",
        cid
      )
      talkState[cid] = nil
    elseif msgcontains(msg, "no") then
