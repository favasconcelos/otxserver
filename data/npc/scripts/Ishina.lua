...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "outfit") then
    if player:getSex() == PLAYERSEX_MALE then
      npcHandler:say(
        "My jewelled belt? <giggles> That's not very manly. Maybe you'd prefer a scimitar like Habdel has.",
        cid
      )
      return true
    end
    ...
    if player:getStorageValue(Storage.OutfitQuest.firstOrientalAddon) < 1 then
      npcHandler:say(
        "My jewelled belt? Of course I could make one for you, but I have a small request. Would you fulfil a task for me?",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      ...
      npcHandler:say(
        "Yeah! That's it! I can't wait to comb my hair! Oh - but first, I'll fulfil my promise: Here is your jewelled belt! Thanks again!",
        cid
      )
      npcHandler.topic[cid] = 0
    end
...
keywordHandler:addKeyword(
  {"need"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I am a jeweller. Maybe you want to have a look at my wonderful {offers}."}
)
keywordHandler:addKeyword(
  {"offers"},
  StdModule.say,
