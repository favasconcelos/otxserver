...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "mission") then
    if
      player:getStorageValue(Storage.thievesGuild.Quest) == 1 and
        player:getStorageValue(Storage.thievesGuild.Mission01) < 1
     then
    ...
    elseif
      player:getStorageValue(Storage.thievesGuild.Quest) == 2 and
        player:getStorageValue(Storage.thievesGuild.Mission02) < 1
     then
    ...
    elseif
      player:getStorageValue(Storage.thievesGuild.Quest) == 3 and
        player:getStorageValue(Storage.thievesGuild.Mission03) < 1
     then
    ...
    elseif
      player:getStorageValue(Storage.thievesGuild.Quest) == 4 and
        player:getStorageValue(Storage.thievesGuild.Mission04) < 1
     then
    ...
    elseif
      player:getStorageValue(Storage.thievesGuild.Quest) == 5 and
        player:getStorageValue(Storage.thievesGuild.Mission05) < 1
     then
      player:setStorageValue(Storage.thievesGuild.Mission05, 1)
      npcHandler:say(
        "Some smugglers underneath Tiquanda, north west of Port Hope owe us some debts. Go there and steal their Golden Goblet and bring it to me.",
        cid
      )
    ...
    elseif
      player:getStorageValue(Storage.thievesGuild.Quest) == 6 and
        player:getStorageValue(Storage.thievesGuild.Mission06) < 1
     then
    ...
    elseif
      player:getStorageValue(Storage.thievesGuild.Quest) == 7 and
        player:getStorageValue(Storage.thievesGuild.Mission07) < 1
     then
    ...
    elseif
      player:getStorageValue(Storage.thievesGuild.Quest) == 8 and
        player:getStorageValue(Storage.thievesGuild.Mission08) < 1
     then
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 2 then
      if player:removeItem(3956, 10) then
        ...
        npcHandler:say(
          "What a fine material. That will be worth a coin or two. So far, so good. Ask me for another mission if you're ready for it.",
          cid
        )
        npcHandler.topic[cid] = 0
      end
    elseif npcHandler.topic[cid] == 3 then
      if player:removeItem(8760, 1) then
        ...
        npcHandler:say(
          "What an ugly vase. But who am I to question the taste of our customers? Anyway, I might have another mission in store for you.",
          cid
        )
        npcHandler.topic[cid] = 0
      end
    ...
    elseif npcHandler.topic[cid] == 5 then
      if player:removeItem(8699, 1) then
        ...
        npcHandler:say(
          "Excellent, that serves this fool right. I fear in your next mission, you'll have to get your hands dirty. Just ask me to learn more about it.",
          cid
        )
        npcHandler.topic[cid] = 0
      end
    elseif npcHandler.topic[cid] == 6 then
      if player:removeItem(8698, 1) then
        ...
        npcHandler:say(
          "That goblet is hardly worth all this trouble but we had to insist on our payment. However, I assume you are eager for more missions, so just ask.",
          cid
        )
        npcHandler.topic[cid] = 0
      end
    elseif npcHandler.topic[cid] == 7 then
      if player:removeItem(8766, 1) then
        ...
        npcHandler:say(
          "This little goldfish will bring us a hefty ransom! Just ask me if you're ready for another mission.",
          cid
        )
        npcHandler.topic[cid] = 0
      end
  ...
  elseif msgcontains(msg, "thieves") then
    if player:getStorageValue(Storage.thievesGuild.Quest) < 1 then
      npcHandler:say(
        "Hm. Well, we could use some fresh blood. Ahum. Do you want to join the thieves guild, |PLAYERNAME|?",
        cid
      )
