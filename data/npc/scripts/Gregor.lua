...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "task") then
    ...
    elseif addonProgress == 2 then
      npcHandler:say(
        "Your current task is to retrieve the helmet of Ramsay the Reckless from Banuta, |PLAYERNAME|.",
        cid
      )
    ...
    elseif addonProgress == 5 then
      npcHandler:say(
        "Please talk to Sam and tell him I sent you. I'm sure he will be glad to refine your helmet, |PLAYERNAME|.",
        cid
      )
    else
  ...
  elseif msgcontains(msg, "behemoth fang") then
    if addonProgress == 1 then
      ...
      npcHandler:say(
        "You're not serious asking that, are you? They come from behemoths, of course. Unless there are behemoth rabbits. Duh.",
        cid
      )
    end
  ...
  elseif msgcontains(msg, "sweat") then
    if addonProgress == 3 then
      ...
      npcHandler:say(
        "Warrior's sweat can be magically extracted from headgear worn by a true warrior, but only in small amounts. Djinns are said to be good at magical extractions.",
        cid
      )
    end
  ...
  elseif npcHandler.topic[cid] == 1 then
    ...
    elseif msgcontains(msg, "no") then
      npcHandler:say(
        "Bah. Then you will have to wait for the day these helmets are sold in shops, but that will not happen before hell freezes over.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 3 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "I'm deeply impressed, brave Knight |PLAYERNAME|. I expected nothing less from you. Now, please retrieve Ramsay's helmet.",
        cid
      )
    elseif msgcontains(msg, "no") then
  ...
  elseif npcHandler.topic[cid] == 4 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Good work, brave Knight |PLAYERNAME|! Even though it is damaged, it has a lot of sentimental value. Now, please bring me warrior's sweat.",
        cid
      )
    elseif msgcontains(msg, "no") then
      npcHandler:say("There is no need to rush anyway.", cid)
  ...
  elseif npcHandler.topic[cid] == 5 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Now that is a pleasant surprise, brave Knight |PLAYERNAME|! There is only one task left now: Obtain royal steel to have your helmet refined.",
        cid
      )
    elseif msgcontains(msg, "no") then
      npcHandler:say("There is no need to rush anyway.", cid)
  ...
  elseif npcHandler.topic[cid] == 6 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "You truly deserve to wear an adorned helmet, brave Knight |PLAYERNAME|. Please talk to Sam and tell him I sent you. I'm sure he will be glad to refine your helmet.",
        cid
      )
    elseif msgcontains(msg, "no") then
      npcHandler:say("There is no need to rush anyway.", cid)
...
keywordHandler:addSpellKeyword(
  {"find", "person"},
  {npcHandler = npcHandler, spellName = "Find Person", price = 80, level = 8, vocation = 4}
)
keywordHandler:addSpellKeyword(
  {"light"},
  {npcHandler = npcHandler, spellName = "Light", price = 0, level = 8, vocation = 4}
)
keywordHandler:addSpellKeyword(
  {"cure", "poison"},
  {npcHandler = npcHandler, spellName = "Cure Poison", price = 150, level = 10, vocation = 4}
)
keywordHandler:addSpellKeyword(
  {"wound", "cleansing"},
  {npcHandler = npcHandler, spellName = "Wound Cleansing", price = 0, level = 8, vocation = 4}
)
keywordHandler:addSpellKeyword(
  {"great", "light"},
  {npcHandler = npcHandler, spellName = "Great Light", price = 500, level = 13, vocation = 4}
)

keywordHandler:addKeyword(
  {"healing", "spells"},
  StdModule.say,
  {npcHandler = npcHandler, text = "In this category I have '{Wound Cleansing}' and '{Cure Poison}'."}
)
keywordHandler:addKeyword(
  {"support", "spells"},
  StdModule.say,
  {npcHandler = npcHandler, text = "In this category I have '{Light}', '{Find Person}' and '{Great Light}'."}
)
keywordHandler:addKeyword(
  {"spells"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"job"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I am the first knight. I trained some of the greatest heroes of Tibia."}
)
keywordHandler:addKeyword(
  {"heroes"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Of course, you heard of them. Knights are the best fighters in Tibia."}
)
keywordHandler:addKeyword({"king"}, StdModule.say, {npcHandler = npcHandler, text = "Hail to our King!"})
keywordHandler:addKeyword(
  {"name"},
  StdModule.say,
  {npcHandler = npcHandler, text = "You are joking, eh? Of course, you know me. I am Gregor, the first knight."}
)
keywordHandler:addKeyword({"gregor"}, StdModule.say, {npcHandler = npcHandler, text = "A great name, isn't it?"})
keywordHandler:addKeyword(
  {"tibia"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Beautiful Tibia. And with our help everyone is save."}
)
...
keywordHandler:addKeyword(
  {"bozo"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Some day someone will make something happen to him..."}
)
keywordHandler:addKeyword(
  {"elane"},
  StdModule.say,
  {npcHandler = npcHandler, text = "A bow might be a fine weapon for someone not strong enough to wield a REAL weapon."}
)
keywordHandler:addKeyword(
  {"frodo"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I and my students often share a cask of beer or wine at Frodo's hut."}
)
keywordHandler:addKeyword(
  {"gorn"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"lynda"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Before she became a priest she won the Miss Tibia contest three times in a row."}
)
keywordHandler:addKeyword({"mcronald"}, StdModule.say, {npcHandler = npcHandler, text = "Peaceful farmers."})
keywordHandler:addKeyword(
  {"ferumbras"},
  StdModule.say,
  {npcHandler = npcHandler, text = "A fine game to hunt. But be careful, he cheats!"}
)
keywordHandler:addKeyword(
  {"muriel"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Bah, go away with these sorcerer tricks. Only cowards use tricks."}
)
keywordHandler:addKeyword({"oswald"}, StdModule.say, {npcHandler = npcHandler, text = "What an idiot."})
keywordHandler:addKeyword(
  {"quentin"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I will never understand this peaceful monks and priests."}
)
keywordHandler:addKeyword(
  {"sam"},
  StdModule.say,
  {npcHandler = npcHandler, text = "He has the muscles, but lacks the guts."}
)
