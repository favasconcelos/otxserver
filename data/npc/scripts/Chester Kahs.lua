...
local function creatureSayCallback(cid, type, msg)
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 3 then
      if player:removeItem(2177, 1) then
        npcHandler:say(
          "Thank you! Take this ring. If you ever need a healing, come, bring the scroll, and ask me to {heal}.",
          cid
        )
    ...
    elseif npcHandler.topic[cid] == 5 then
      ...
      npcHandler:say(
        "Then I welcome you to the TBI. This is a great moment for you, remember it well. Talk to me about your missions whenever you feel ready.",
        cid
      )
      npcHandler.topic[cid] = 0
    elseif npcHandler.topic[cid] == 6 then
      ...
      npcHandler:say(
        "I think they understood the warning the way it was meant. If not, you will have to visit Venore soon again. But for now it's settled.",
        cid
      )
      npcHandler.topic[cid] = 0
    ...
    elseif npcHandler.topic[cid] == 8 then
      if player:removeItem(14324, 1) then
        ...
        npcHandler:say(
          "I can only hope that this information are as valuable as we expected it. A good man died for them.",
          cid
        )
      else
    ...
    elseif npcHandler.topic[cid] == 9 then
      if player:removeItem(14325, 1) then
        ...
        npcHandler:say(
          "Ah yes, very interesting. Almost as I suspected. It's a good thing that we got those documents in our hands.",
          cid
        )
      else
    ...
    elseif npcHandler.topic[cid] == 11 then
      ...
      npcHandler:say(
        "I already heard that our little trick worked quite well. Several officials of Carlin are already on their way to repair the damage done to their diplomatic efforts. It will not only cost them much money but also quite some time.",
        cid
      )
      npcHandler.topic[cid] = 0
    ...
    elseif npcHandler.topic[cid] == 13 then
      if player:removeItem(7699, 1) then
        ...
        npcHandler:say(
          "You have done superb work agent, I grant you the title of Top Agent! Here's a little gift you might find useful.",
          cid
        )
      else
  ...
  elseif npcHandler.topic[cid] == 2 then
    if
      msgcontains(msg, "magic") and msgcontains(msg, "crystal") and msgcontains(msg, "lugri") and
        msgcontains(msg, "deathcurse")
     then
  ...
  elseif msgcontains(msg, "mission") then
    if
      player:getStorageValue(Storage.secretService.Quest) == 1 and
        player:getStorageValue(Storage.secretService.AVINMission01) < 1 and
        player:getStorageValue(Storage.secretService.CGBMission01) < 1
     then
    ...
    elseif
      player:getStorageValue(Storage.secretService.TBIMission01) == 3 and
        player:getStorageValue(Storage.secretService.Quest) == 3
     then
    ...
    elseif
      player:getStorageValue(Storage.secretService.TBIMission02) == 2 and
        player:getStorageValue(Storage.secretService.Quest) == 5
     then
    ...
    elseif
      player:getStorageValue(Storage.secretService.TBIMission03) == 3 and
        player:getStorageValue(Storage.secretService.Quest) == 7
     then
    ...
    elseif
      player:getStorageValue(Storage.secretService.TBIMission04) == 2 and
        player:getStorageValue(Storage.secretService.Quest) == 9
     then
    ...
    elseif
      player:getStorageValue(Storage.secretService.TBIMission05) == 3 and
        player:getStorageValue(Storage.secretService.Quest) == 11
     then
    ...
    elseif
      player:getStorageValue(Storage.secretService.TBIMission06) == 3 and
        player:getStorageValue(Storage.secretService.Quest) == 13
     then
    ...
    elseif
      player:getStorageValue(Storage.secretService.TBIMission06) == 3 and
        player:getStorageValue(Storage.secretService.Mission07) == 1
     then
      npcHandler:say("Have you fulfilled your current mission?", cid)
  ...
  elseif msgcontains(msg, "disguise") then
    if player:getStorageValue(Storage.secretService.TBIMission06) == 1 then
      npcHandler:say(
        "If you lost or wasted your disguise kit I can replace it. It will cost you 1000 gold though since you lost royal property. Is that ok for you?",
        cid
      )
