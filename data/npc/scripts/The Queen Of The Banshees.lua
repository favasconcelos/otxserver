...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "seventh seal") then
    npcHandler:say(
      "If you have passed the first six seals and entered the blue fires that lead to the chamber of the seal you might receive my {kiss} ... It will open the last seal. Do you think you are ready?",
      cid
    )
  ...
  elseif msgcontains(msg, "kiss") and npcHandler.topic[cid] == 7 then
    if player:getStorageValue(Storage.QueenOfBansheesQuest.Kiss) < 1 then
      npcHandler:say(
        "Are you prepared to receive my kiss, even though this will mean that your death as well as a part of your soul will forever belong to me, my dear?",
        cid
      )
  ...
  elseif msgcontains(msg, "spectral dress") then
    if
      player:getStorageValue(Storage.ExplorerSociety.QuestLine) == 48 and
        player:getStorageValue(Storage.ExplorerSociety.bansheeDoor) < 1
     then
      npcHandler:say(
        "Your wish for a spectral dress is silly. Although I will grant you the permission to take one. My maidens left one in a box in a room, directly south of here.",
        cid
      )
  ...
  elseif msgcontains(msg, "addon") then
    if player:getStorageValue(Storage.OutfitQuest.WizardAddon) == 5 then
      npcHandler:say(
        "Say... I have been longing for something for an eternity now... if you help me retrieve it, I will reward you. Do you consent to this arrangement?",
        cid
      )
  ...
  elseif msgcontains(msg, "yes") then
    ...
    elseif npcHandler.topic[cid] == 8 then
      if not player:isPzLocked() then
        ...
        npcHandler:say(
          "You have spilled too much blood recently and the dead are hungry for your soul. Perhaps return when you regained you inner balance.",
          cid
        )
        npcHandler.topic[cid] = 0
      end
    ...
    elseif npcHandler.topic[cid] == 11 then
      if player:removeItem(5922, 50) then
        npcHandler:say(
          "Thank you! You have no idea what that means to me. As promised,here is your reward... as a follower of Zathroth, I hope that you will like this accessory.",
          cid
        )
...
keywordHandler:addKeyword(
  {"stay"},
  StdModule.say,
  {npcHandler = npcHandler, text = "It's my curse to be the eternal {guardian} of this ancient {place}."}
)
keywordHandler:addKeyword(
  {"guardian"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"banshee"},
  StdModule.say,
  {npcHandler = npcHandler, text = "They are my maidens. They give me comfort in my eternal watch over the last seal."}
)
