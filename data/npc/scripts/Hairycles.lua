...
local function greetCallback(cid)
  if Player(cid):getStorageValue(Storage.TheApeCity.Questline) < 12 then
    ...
    npcHandler:setMessage(
      MESSAGE_GREET,
      "Be greeted, friend of the ape people. If you want to {trade}, just ask for my offers. If you are injured, ask for healing."
    )
  end
  return true
end
...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "mission") then
    if questProgress < 1 then
      npcHandler:say(
        "These are dire times for our people. Problems plenty are in this times. But me people not grant trust easy. Are you willing to prove you friend of ape people?",
        cid
      )
    ...
    elseif questProgress == 1 then
      if player:getStorageValue(Storage.QuestChests.WhisperMoss) == 1 then
        ...
        npcHandler:say(
          "Please hurry. Bring me whisper moss from dworc lair. Make sure it is from dworc lair! Take it yourself only! If you need to hear background of all again, ask Hairycles for {background}.",
          cid
        )
      end
    ...
    elseif questProgress == 4 then
      npcHandler:say(
        "Little ape should be healthy soon. Me so happy is. Thank you again! But me suspect we in more trouble than we thought. Will you help us again?",
        cid
      )
    ...
    elseif questProgress == 13 then
      if player:getStorageValue(Storage.TheApeCity.Casks) == 3 then
        npcHandler:say(
          "You do please Hairycles again, friend. Me hope madness will not spread further now. Perhaps you are ready for other mission.",
          cid
        )
        player:setStorageValue(Storage.TheApeCity.Questline, 14)
      else
        npcHandler:say(
          "Please destroy three casks in the complex beneath Banuta, so my people will come to senses again.",
          cid
        )
      end
    ...
    elseif questProgress == 17 then
      if player:getStorageValue(Storage.TheApeCity.SnakeDestroyer) == 1 then
        ...
        npcHandler:say(
          "Me know its much me asked for but go into the deepest catacombs under Banuta and destroy the monument of the serpent god.",
          cid
        )
      end
    else
      npcHandler:say(
        "No more missions await you right now, friend. Perhaps you want to check me offers for special friends... or shamanic powers.",
        cid
      )
    end
  ...
  elseif msgcontains(msg, "outfit") or msgcontains(msg, "shamanic") then
    if questProgress == 18 then
      if player:getStorageValue(Storage.TheApeCity.ShamanOutfit) ~= 1 then
        npcHandler:say(
          "Me truly proud of you, friend. You learn many about plants, charms and ape people. Me want grant you shamanic power now. You ready?",
          cid
        )
  ...
  elseif npcHandler.topic[cid] == 1 then
    if msgcontains(msg, "yes") then
      npcHandler:say(
        "To become friend of ape people a long and difficult way is. We do not trust easy but help is needed. Will you listen to story of Hairycles?",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 3 then
    if msgcontains(msg, "yes") then
      if not player:removeItem(4838, 1) then
        npcHandler:say(
          "Stupid, you no have the moss me need. Go get it. It's somewhere in dworc lair. If you lost it, they might restocked it meanwhile. If you need to hear background of all again, ask Hairycles for {background}.",
          cid
        )
      ...
      npcHandler:say(
        "Ah yes! That's it. Thank you for bringing mighty whisper moss to Hairycles. It will help but still much is to be done. Just ask for other mission if you ready.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 4 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "You so good! Brought syrup to me! Thank you, will prepare cure now. Just ask for {mission} if you want help again.",
        cid
      )
    ...
    if msgcontains(msg, "yes") then
      if not player:removeItem(5956, 1) then
        if player:getStorageValue(Storage.QuestChests.OldParchment) == 1 then
          npcHandler:say(
            "That's bad news. If you lost it, only way to get other is to kill holy serpents. But you can't go there so you must ask adventurers who can.",
            cid
          )
        else
          npcHandler:say(
            "No! That not scroll me looking for. Silly hairless ape you are. Go to village of lizards and get it there on your own!",
            cid
          )
        end
        return true
      ...
      npcHandler:say(
        "You brought scroll with lizard text? Good! I will see what text tells me! Come back when ready for other mission.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 9 then
    if msgcontains(msg, "yes") then
      npcHandler:say(
        "Oh, so clear is all now! Easy it will be to read the signs now! Soon we will know what to do! Thank you again! Ask for mission if you feel ready.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 10 then
    if msgcontains(msg, "yes") then
      npcHandler:say(
        "You brave hairless ape! Get me hydra egg. If you lose egg, you probably have to fight many, many hydras to get another.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 11 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Ah, the egg! Mighty warrior you be! Thank you. Hairycles will put it at safe place immediately.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 13 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Incredible, you brought a witches' cap! Now me can prepare mighty charm of life. Yet still other {missions} will await you, friend.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 16 then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Incredible! You got a hair of holy Bong! This will raise the spirit of my people. You are truly a friend. But one last mission awaits you.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 17 then
    if msgcontains(msg, "yes") then
      npcHandler:say(
        "Hairycles sure you will make it. Just use hammer on all that looks like snake or lizard. Tell Hairycles if you succeed with mission.",
        cid
      )
  ...
  elseif npcHandler.topic[cid] == 18 then
    if msgcontains(msg, "yes") then
      npcHandler:say(
        "Friend of the ape people! Take my gift and become me apprentice! Here is shaman clothing for you!",
        cid
      )
...
keywordHandler:addKeyword(
  {"things"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Things not good now. Need helper to do {mission} for me people."}
)
keywordHandler:addKeyword(
  {"ape people"},
  StdModule.say,
...
keywordHandler:addKeyword(
  {"sibang"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Sibang verry fast and funny. Sibang good gather food. Sibang know {jungle} well."}
)
keywordHandler:addKeyword(
  {"merlkin"},
  StdModule.say,
