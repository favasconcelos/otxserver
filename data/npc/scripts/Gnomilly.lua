...
function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "job") then
    return npcHandler:say(
      "I'm the officer responsible for this area. I give out missions, accept mission reports and oversee our defences.",
      cid
    )
  end
  ...
  if msgcontains(msg, "gnome") then
    return npcHandler:say(
      "We are the only protectors of the world against the enemies below. With small stature comes great responsibilities, as they say.",
      cid
    )
  end
  ...
  if msgcontains(msg, "mission") then
    if player:getLevel() > levels[2] then
      npcHandler:say(
        "Sorry, but no! Your expertise could be put to better use elsewhere. Here awaits you no challenge. You are desperately needed in the deeper levels of the Spike. Report there immediately. ",
        cid
      )
    else
      npcHandler:say(
        "I can offer you several missions: to recharge our ghost {pacifiers}, to {release} the spiritual anger, to {track} an evil presence and to {kill} some demon skeletons.",
        cid
      )
    end
    return
  ...
  if msgcontains(msg, "report") then
    ...
    return npcHandler:say(
      "What mission do you want to report about: recharging the ghost {pacifiers}, the {release} of the spiritual anger, about {tracking} an evil presence and the {killing} of demon skeletons?",
      cid
    )
  end
  ...
  if msgcontains(msg, "pacifiers") then
    if player:getExhaustion(SPIKE_UPPER_PACIFIER_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_UPPER_PACIFIER_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if (player:getLevel() < levels[1]) or (player:getLevel() > levels[2]) then
      return npcHandler:say(
        "Sorry, you are not on the required range of levels [" .. levels[1] .. "-" .. levels[2] .. "].",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_UPPER_PACIFIER_MAIN) == -1 then
      npcHandler:say(
        {
          "We need you to recharge our ghost pacifiers. They are placed at several strategic points in the caves around us and should be easy to find. Your mission would be to charge seven of them.",
  ...
  if talkState[cid] == "pacifiers" then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Gnometastic! Take this resonance charger and use it on seven of the pacifiers in the cave. If you lose the charger, you'll have to bring your own. Gnomux sells all the equipment that is required for our missions.",
        cid
      )
      talkState[cid] = nil
  ...
  if msgcontains(msg, "release") then
    if player:getExhaustion(SPIKE_UPPER_MOUND_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_UPPER_MOUND_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_UPPER_MOUND_MAIN) == -1 then
      npcHandler:say(
        "Your task would be to use a spirit shovel to release some spirit's anger from graves that can be found all around here. If you are interested, I can give you some more information about it. Are you willing to accept this mission?",
        cid
      )
  ...
  if talkState[cid] == "release" then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Gnometastic! Take this spirit shovel and use it on four graves in the cave system. If you lose the shovel you'll have to bring your own. Gnomux sells all the equipment that is required for our missions.",
        cid
      )
      talkState[cid] = nil
    elseif msgcontains(msg, "no") then
  ...
  if msgcontains(msg, "track") then
    if player:getExhaustion(SPIKE_UPPER_TRACK_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_UPPER_TRACK_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_UPPER_TRACK_MAIN) == -1 then
      npcHandler:say(
        {
          "You'd be given the highly important task to track down an enormously malevolent spiritual presence in the cave system. Use your tracking device to find out how close you are to the presence.",
          "Use that information to find the residual energy and use the tracker there. If you are interested, I can give you some more information about it. Are you willing to accept this mission?"
        },
        cid
      )
      talkState[cid] = "track"
  ...
  if talkState[cid] == "track" then
    if msgcontains(msg, "yes") then
      ...
      npcHandler:say(
        "Gnometastic! Use this tracking device in the caves and locate the residual spirit energy. If you lose the tracking device, you'll have to bring your own. Gnomux sells all the equipment that is required for our missions.",
        cid
      )
      talkState[cid] = nil
    elseif msgcontains(msg, "no") then
  ...
  if msgcontains(msg, "kill") then
    if player:getExhaustion(SPIKE_UPPER_KILL_DAILY) > 0 then
      return npcHandler:say(
        "Sorry, you have to wait " ..
          string.diff(player:getStorageValue(SPIKE_UPPER_KILL_DAILY) - os.time()) ..
            " before this task gets available again.",
        cid
      )
    end
    ...
    if player:getStorageValue(SPIKE_UPPER_KILL_MAIN) == -1 then
      npcHandler:say(
        "We need someone to reduce the steadily growing number of demon skeletons in the caves. If you are interested, I can give you some more information about it. Are you willing to accept this mission?",
        cid
      )
      talkState[cid] = "kill"
    else
