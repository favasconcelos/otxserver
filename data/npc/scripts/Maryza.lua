...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "cookbook") then
    if player:getStorageValue(Storage.MaryzaCookbook) ~= 1 then
      npcHandler:say(
        "The cookbook of the famous dwarven kitchen. You're lucky. I have a few copies on sale. Do you like one for 150 gold?",
        cid
      )
      npcHandler.topic[cid] = 1
    else
      npcHandler:say(
        "I'm sorry but I sell only one copy to each customer. Otherwise they would have been sold out a long time ago.",
        cid
      )
    end
