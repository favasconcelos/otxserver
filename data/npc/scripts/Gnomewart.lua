...
local function creatureSayCallback(cid, type, msg)
  ...
  if msgcontains(msg, "endurance") then
    if
      player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 9 and
        player:getStorageValue(Storage.BigfootBurden.QuestLine) <= 10
     then
      npcHandler:say(
        {
    ...
    elseif player:getStorageValue(Storage.BigfootBurden.QuestLine) == 11 then
      npcHandler:say(
        "You have passed the test and are ready to create your soul melody. Talk to Gnomelvis in the east about it.",
        cid
      )
