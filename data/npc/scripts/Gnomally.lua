...
local function onBuy(cid, item, subType, amount, ignoreCap, inBackpacks)
  ...
  player:sendTextMessage(
    MESSAGE_INFO_DESCR,
    string.format("Bought %dx %s for %d gold.", count, items[item].realName, items[item].buyPrice * count)
  )
  return true
end
...
local function greetCallback(cid)
  npcHandler:setMessage(
    MESSAGE_GREET,
    "Oh, hello! I'm the gnome-human relations assistant. I am here for you to trade your tokens for {equipment}, resupply you with mission {items} and talk to you about your {relations} to us gnomes! ..."
  )
  return true
end
...
local function creatureSayCallback(cid, type, msg)
  ...
  elseif config[msg] then
    ...
    npcHandler:say(
      string.format(
        "Do you want to trade %s %s for %d %s tokens?",
        (itemType:getArticle() ~= "" and itemType:getArticle() or ""),
        itemType:getName(),
        config[msg].token.count,
        config[msg].token.type
      ),
      cid
    )
  ...
  elseif msgcontains(msg, "relations") then
    ...
    if player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 14 then
      npcHandler:say(
        "Our relations improve with every mission you undertake on our behalf. Another way to improve your relations with us gnomes is to trade in minor crystal tokens. ...",
        cid
      )
      npcHandler:say(
        "Your renown amongst us gnomes is currently {" ..
          math.max(0, player:getStorageValue(Storage.BigfootBurden.Rank)) ..
            "}. Do you want to improve your standing by sacrificing tokens? One token will raise your renown by 5 points. ",
        cid
      )
    ...
    elseif npcHandler.topic[cid] == 4 then
      ...
      if player:removeItem(18422, renown[cid]) then
        player:setStorageValue(
          Storage.BigfootBurden.Rank,
          math.max(0, player:getStorageValue(Storage.BigfootBurden.Rank)) + renown[cid] * 5
        )
        npcHandler:say(
          "As you wish! Your new renown is {" .. player:getStorageValue(Storage.BigfootBurden.Rank) .. "}.",
          cid
        )
      else
