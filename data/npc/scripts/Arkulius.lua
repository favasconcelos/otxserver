...
local function creatureSayCallback(cid, type, msg)
  ...
  elseif msgcontains(msg, "mission") or msgcontains(msg, "quest") then
    ...
    elseif value == 1 then
      if
        player:getItemCount(
          player:isSorcerer() and 8304 or player:isDruid() and 8305 or player:isPaladin() and 8300 or
            player:isKnight() and 8306
        ) > 0
       then
        ...
        npcHandler:say(
          {
            "Impressive!! Let me take a look.......Ahh, " ..
              (player:isSorcerer() and
                "an ETERNAL FLAME! Now you need to find a knight, a druid, and a paladin who also completed this first task. ..." or
                player:isDruid() and
                  "MOTHER SOIL! Now you need to find a knight, a paladin, and a sorcerer who also completed this first task. ..." or
                player:isPaladin() and
                  "a FLAWLESS ICE CRYSTAL! Now you need to find a knight, a druid, and a sorcerer who also completed this first task. ..." or
                player:isKnight() and
        ...
        npcHandler:say(
          "You need some kind of pure elemental soil from the " ..
            (player:isSorcerer() and "Fire" or player:isDruid() and "Earth" or player:isPaladin() and "Ice" or
              player:isKnight() and "Energy") ..
              " Overlord. Come back when you've got it.",
          cid
        )
      end
      npcHandler.topic[cid] = 0
    elseif value == 2 then
      if player:removeItem(8310, 1) and player:getStorageValue(Storage.ElementalSphere.QuestLine) < 3 then
        npcHandler:say(
          "AMAZING!! I'm going to start immediately with the research. If it turns out the way I expect it, Alverus will be revived soon!! Here, take this as a reward and try to collect more of this substance. I'll make you a good offer, I promise. ",
          cid
        )
        player:addItem(
          player:isSorcerer() and 8867 or player:isDruid() and 8869 or player:isPaladin() and 8853 or
            player:isKnight() and 8883,
          1
        )
...
keywordHandler:addKeyword(
  ...
  StdModule.say,
  {
    ...
    text = "How dare you asking me this?!? I'm Arkulius - Master of Elements, the HEADMASTER of this academy!!"
  }
)
keywordHandler:addKeyword(
  {"name"},
  StdModule.say,
  {npcHandler = npcHandler, text = "I'm Arkulius - Master of Elements, the headmaster of this academy."}
)
keywordHandler:addKeyword(
  {"help"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "I have better things to do than helping you. See that ice statue over there? My dear friend Alverus needs to be revived!"
  }
)
keywordHandler:addKeyword(
  {"time"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Time is an illusion and completely irrelevant to me."}
)
keywordHandler:addKeyword(
  {"weapon"},
  StdModule.say,
  {
    npcHandler = npcHandler,
    text = "Weapons are for those people who aren't able to use their heads or better what's INSIDE their heads. No offence <coughs>."
  }
) -- < Knight; FIXME !!!
keywordHandler:addKeyword(
  {"pits of inferno"},
  StdModule.say,
  {npcHandler = npcHandler, text = "Yeye, I believe you almost feel like home among all those brainless creatures!"}
)
