...
local function creatureSayCallback(cid, type, msg)
  ...
  if
    msgcontains(msg, "trouble") and player:getStorageValue(Storage.TheInquisition.TimGuard) < 1 and
      player:getStorageValue(Storage.TheInquisition.Mission01) ~= -1
   then
  ...
  elseif msgcontains(msg, "authorities") then
    if npcHandler.topic[cid] == 1 then
      npcHandler:say(
        "What do you mean? Of course they will immediately send someone with extra long and thin arms to retrieve it! ",
        cid
      )
  ...
  elseif msgcontains(msg, "avoided") then
    if npcHandler.topic[cid] == 2 then
      npcHandler:say(
        "Your humour might let end you up beaten in some dark alley, you know? No, I don't think someone could have prevented that accident! ",
        cid
      )
    ...
    if npcHandler.topic[cid] == 3 then
      ...
      if player:getStorageValue(Storage.TheInquisition.TimGuard) < 1 then
        ...
        player:setStorageValue(
          ...
          player:getStorageValue(Storage.TheInquisition.Mission01) + 1
        ) -- The Inquisition Questlog- "Mission 1: Interrogation"
