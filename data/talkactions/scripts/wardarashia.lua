function onSay(cid, words, param)
  ...
  if param:find('invite') == 1 and 3 then
    ...
    if player == nil or tonumber(minutos) == nil or player == '' or tonumber(minutos) == 0 or onlysd == nil or noue == nil then
      for i = 1, #status do
        doPlayerSendTextMessage(
          cid,
          status[i],
          '[WAR AREA] Use: !wardara "invite NOME_DE_UM_MEMBRO_INIMIGO MINUTOS_DE_WAR SEMSD SEMUE (EX: !wardara "invite Charles 40 - invitar para uma war de 40 minutos)'
        )
      end
      return false
    end
    ...
    for i = 1, #status do
      doPlayerSendTextMessage(
        cid,
        status[i],
        '[WAR AREA] Voce invitou a guild ' .. guild .. ' para ' .. tonumber(minutos) .. ' minutos, o lider da ' .. guild .. ' tem 1 minuto para aceitar seu pedido!.'
      )
      if (getPlayerGuildLevel(tid) == 3) then
        doPlayerSendTextMessage(
          tid,
          status[i],
          '[WAR AREA] Sua guild recebeu um convite da guild ' ..
            inguild .. ' para ' .. tonumber(minutos) .. ' minutos na Area de Darashia, voce tem 1 minuto para usar !wardara "accept para aceitar ou !wardara "reject para rejeitar!.'
        )
      else
  ...
  elseif param:lower():find('join') == 1 then
    if
      (getGlobalStorageValue(config.playerstorage) == 1 and getGlobalStorageValue(config.osplayerstorage) > os.time() and getPlayerStorageValue(cid, config.hplayerstorage) ~= 2) and
        (gid == guildone or gid == guildtwo)
     then
