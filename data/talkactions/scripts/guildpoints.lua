...
local function getValidAccounts(guild)
  local resultId =
    db.storeQuery(
    'SELECT a.`id` FROM `accounts` a, `guild_membership` m, `players` p WHERE m.`guild_id` = ' ..
      guild:getId() .. ' AND p.`id` = m.`player_id` AND p.`level` > ' .. config.minimumLevel .. ' and a.`id` = p.`account_id` AND a.`guild_points_stats` = 0 GROUP BY a.`id`;'
  )
...
function onSay(player, words, param)
  ...
  if #members < config.membersNeeded then
    ...
    player:sendTextMessage(
      ...
      'Only ' .. #members .. ' guild members online, you need ' .. config.membersNeeded .. ' guild members with level ' .. config.minimumLevel .. ' or higher.'
    )
    return false
  end
  ...
  if #validAccounts > 0 then
    db.query(
      'UPDATE `accounts` SET `guild_points` = `guild_points` + ' .. config.pointAmount .. ', `guild_points_stats` = ' .. os.time() .. ' WHERE `id` IN (' .. table.concat(validAccounts, ',') .. ');'
    )
    for i = 1, #members do
