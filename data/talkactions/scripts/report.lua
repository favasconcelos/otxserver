function onSay(player, words, param)
  ...
  if (Player.getExhaustion(player, 5804783) <= 0) then
    ...
    if (string.len(description) == 0) then
      player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, 'Por favor, especifique o seu problema. Ex: !report SQM BUG')
      return false
    end

    if (string.len(description) > 130) then
      player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, 'Por favor, digite no máximo 130 caracteres na descrição!')
      return false
    end

    Player.setExhaustion(player, 5804783, 60)
    db.asyncQuery(
      'INSERT INTO support (id, name, description, posx, posy, posz, status) VALUES (0, ' ..
        db.escapeString(player:getName()) .. ', ' .. db.escapeString(description) .. ', ' .. posReport.x .. ', ' .. posReport.y .. ', ' .. posReport.z .. ', ' .. db.escapeString('PENDENTE') .. ')'
    )
