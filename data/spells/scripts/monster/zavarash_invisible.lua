...
function onCastSpell(cid, var)
  ...
  if
    getTileItemById(cpos, 1490).uid ~= 0 or getTileItemById(cpos, 1496).uid ~= 0 or getTileItemById(cpos, 1503).uid ~= 0
   then
    doSetCreatureOutfit(
      cid,
      {lookType = 12, lookHead = 0, lookAddons = 0, lookLegs = 57, lookBody = 15, lookFeet = 85},
      -1
    )
