...
function onSpeak(player, type, message)
  ...
  if
    (getCreatureCondition(player, CONDITION_CHANNELMUTEDTICKS, CHANNEL_HELP)) and
      playerAccountType == ACCOUNT_TYPE_NORMAL
   then
        ...
        if playerAccountType > target:getAccountType() then
          if not target:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_HELP) then
            ...
            sendChannelMessage(
              ...
              TALKTYPE_CHANNEL_R1,
              target:getName() ..
                " has been muted by " .. player:getName() .. " for using Help Channel inappropriately."
            )
          else
