...
-- ### END OF CONFIG ###
MESSAGE_ERROR_CONTACT = "[ERRO] Shop Donate. Contate o Administrador! Erro dispon�vel no Console."
  ...
  if resultId ~= false then
    repeat
      ...
      if player then
        ...
        -- DELIVER ITEM
        if shopOfferType == "item" or shopOfferType == "itemvip" then
          ...
          if not playerStoreInbox then
            ...
            print("[ERRO] Shop Donate (" .. player:getName() .. ') - n�o foi encontrado seu "Store Inbox".')
            return true
          end
          ...
          if type(receivedItemStatus) == "number" and receivedItemStatus == RETURNVALUE_NOERROR then
            player:sendTextMessage(
              SHOP_MSG_TYPE,
              "Voc� recebeu " .. shopOfferName .. " do Store. Voc� pode encontrar esse item no seu Store Inbox."
            )
            ...
            db.asyncQuery(
              "UPDATE `z_shop_history_item` SET `trans_state`= 'realized', `trans_real`=" ..
                os.time() .. " WHERE `id` = " .. transactionId
            )
          else
            player:sendTextMessage(SHOP_MSG_TYPE, MESSAGE_ERROR_CONTACT)
            print(
              "[ERRO] Shop Donate (" ..
                player:getName() ..
                  ") - n�o pode adicionar item no Store Inbox - raz�o desconhecida, provavel inbox cheio? - ITEM ID: " ..
                    itemId .. ", ITEM COUNT: " .. itemCount
            )
          end
        elseif shopOfferType == "vipdays" then
          ...
          player:sendTextMessage(
            SHOP_MSG_TYPE,
            "Voc� recebeu " .. shopOfferName .. " do Store. Agora voc� tem " .. vipDays .. " de dias!"
          )
          db.asyncQuery("DELETE FROM `z_ots_comunication` WHERE `id` = " .. transactionId)
          db.asyncQuery(
            "UPDATE `z_shop_history_item` SET `trans_state`= 'realized', `trans_real`=" ..
        ...
        elseif shopOfferType == "container" then
          ...
          if not newContainerUID or not Container(newContainerUID) then
            ...
            print(
              "[ERRO] Shop Donate (" ..
                player:getName() .. ") - n�o pode criar Container - ID inv�lido - CONTAINER ID:" .. containerId
            )
            return true
          end
          ...
          for i = 1, containerItemsInsideCount do
            ...
            if not newItemUID then
              ...
              print(
                "[ERRO] Shop Donate (" ..
                  player:getName() ..
                    ") - n�o pode criar Item - ID inv�lido ou Count - ITEM ID: " ..
                      itemId .. ", ITEM COUNT: " .. itemCount
              )
              return true
            end
            ...
            if type(addItemToContainerResult) ~= "number" or addItemToContainerResult ~= RETURNVALUE_NOERROR then
              ...
              print(
                "[ERRO] Shop Donate (" ..
                  player:getName() ..
                    ') - n�o pode adicionar item no Container - item n�o � peg�vel OU vari�vel "RETURNVALUE_NOERROR" n�o existe em LUA - ITEM ID: ' ..
                      itemId .. ", ITEM COUNT: " .. itemCount
              )
              return true
            end
          ...
          if not playerStoreInbox then
            ...
            print(
              "[ERRO] Shop Donate (" ..
                player:getName() ..
                  ') - n�o pode abrir Store Inbox - it is not supported in your server OR variable "CONST_SLOT_STORE_INBOX" is not definied in LUA'
            )
            return true
          end
          -- add container with items to Store Inbox
          ...
          if type(receivedItemStatus) == "number" and receivedItemStatus == RETURNVALUE_NOERROR then
            player:sendTextMessage(
              SHOP_MSG_TYPE,
              "You received " ..
                shopOfferName .. " from Website Shop. You can find your item in STORE INBOX (under EQ)."
            )
            db.asyncQuery("DELETE FROM `z_ots_comunication` WHERE `id` = " .. transactionId)
            db.asyncQuery(
              "UPDATE `z_shop_history_item` SET `trans_state`= 'realized', `trans_real`=" ..
            ...
            print(
              "[ERRO] Shop Donate (" ..
                player:getName() ..
                  ") - n�o pode adicionar Container com itens no Store Inbox - unknown reason, is it's size limited and it is full? - ITEM ID: " ..
                    itemId ..
                      ", ITEM COUNT: " ..
                        itemCount ..
                          ", CONTAINER ID:" .. containerId .. ", ITEMS IN CONTAINER COUNT:" .. containerItemsInsideCount
            )
          end
        elseif shopOfferType == "mount" then -- addon, mount etc.
          ...
          player:sendTextMessage(SHOP_MSG_TYPE, "Voc� recebeu " .. shopOfferName .. " do Shop Donate!")
          db.asyncQuery("DELETE FROM `z_ots_comunication` WHERE `id` = " .. transactionId)
          db.asyncQuery(
            "UPDATE `z_shop_history_item` SET `trans_state`= 'realized', `trans_real`=" ..
              os.time() .. " WHERE `id` = " .. transactionId
