...
local function lmsclosed1()
  broadcastMessage(
    "LMS event will start in 5 minutes. Portal is opened in north of depot Thais. The reward is 2 PREMIUM POINTS, Hurry!!",
...
local function lmsclosed()
  ...
  if (count >= MinimumPlayers) then
    ...
    broadcastMessage(
      "Not enough players to start Last man Standing event! Minimum: " .. MinimumPlayers .. " players.",
      MESSAGE_EVENT_ADVANCE
    )
    ...
    if spectators ~= nil then
      for _, spectator in ipairs(spectators) do
        if isPlayer(spectator) then
          ...
          doSendMagicEffect(depotcenter, CONST_ME_TELEPORT)
          broadcastMessage(
            "LMS ended winner is: " .. getPlayerName(spectator) .. ", reward is 2 premium points.",
            MESSAGE_EVENT_ADVANCE
          )
          ...
          db.query(
            "UPDATE `accounts` SET `premium_points` = `premium_points` + " .. points .. " WHERE `id` = " .. accid
          )
        end
      end
    end
...
local function lms()
  broadcastMessage(
    "LMS event will start in 9 minutes. Portal is opened in Event Room [Temple of Thais]. The reward is 2 PREMIUM POINTS, Hurry!!",
    MESSAGE_EVENT_ADVANCE
  )
  ...
  addEvent(
    function()
      doRemoveItem(getTileItemById(portalwhere, 11796).uid)
    end,
    600 * 1000
  )
  addEvent(
