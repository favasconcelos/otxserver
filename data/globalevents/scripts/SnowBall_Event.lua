function startGame(rounds)
  if rounds == 0 then
    if #CACHE_GAMEPLAYERS < SnowBall_Configurations.Event_MinPlayers then
      ...
      broadcastMessage(
        "[Snowball Event]: O evento foi cancelado por conta de n�o ter no minimo " ..
          SnowBall_Configurations.Event_MinPlayers .. " jogadores.",
      ...
      broadcastMessage("[Snowball Event]: O evento foi fechado. O jogo come�ou.", MESSAGE_EVENT_ADVANCE)
  ...
  if #CACHE_GAMEPLAYERS < SnowBall_Configurations.Event_MinPlayers then
    broadcastMessage(
      "[Snowball Event]: Faltam " ..
        rounds ..
          " minuto(s) e " ..
            SnowBall_Configurations.Event_MinPlayers - #CACHE_GAMEPLAYERS .. " jogador(es) para o jogo come�ar.",
      MESSAGE_EVENT_ADVANCE
    )
  else
    broadcastMessage("[Snowball Event]: Faltam " .. rounds .. " minuto(s) para o jogo come�ar.", MESSAGE_EVENT_ADVANCE)
  end
...
function onTime(interval)
  ...
  broadcastMessage(
    "[Snowball Event]: O evento foi aberto, v� at� o EVENT ROOM no templo de Thais para participar.",
    MESSAGE_EVENT_ADVANCE
  )
