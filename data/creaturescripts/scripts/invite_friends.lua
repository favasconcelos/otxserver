  ...
  if getInvitePoints(player) > 0 then
    ...
    for vod, ka in pairs(_invite_friends.rewards) do
      ...
      if FriendPoints >= vod and CheckPoints < vod then
        str =
          str ..
          "--> Invite Players System <--\n\nVoc� acaba de receber algumas recompensas:\n\nItems: \n" ..
        ...
        player:sendTextMessage(
          ...
          "[Invite Players System] Voc� Recebeu algumas premia��es por estar convidando jogadores para o servidor, Por favor conferir os itens no Depot."
        )
      end
    end
  end
...
function onAdvance(player, skill, oldlevel, newlevel)
  if (skill == SKILL__LEVEL) then
    if
      hasInviteFriend(player) and player:getStorageValue(_invite_friends.storages[3]) < newLevel and
        _invite_friends.levels_win[newLevel]
     then
      ...
      player:sendTextMessage(
        MESSAGE_STATUS_CONSOLE_ORANGE,
        "[Invite Friends] voc� atingiu o level " ..
          newLevel .. " e o seu amigou " .. f_name .. " recebeu " .. points .. " Friend Points. Obrigado!"
      )
    end
  end
  return true
