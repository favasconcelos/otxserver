...
function onPrepareDeath(creature, lastHitKiller, mostDamageKiller)
  if
    creature:isPlayer() and creature:getStorageValue(_Lib_Battle_Info.TeamOne.storage) >= 1 or
      creature:getStorageValue(_Lib_Battle_Info.TeamTwo.storage) >= 1
   then
    local MyTeam =
    ...
    local EnemyTeam =
      creature:getStorageValue(_Lib_Battle_Info.TeamOne.storage) >= 1 and _Lib_Battle_Info.TeamTwo.storage or
      _Lib_Battle_Info.TeamOne.storage
      ...
      doBroadCastBattle(
        ...
        "[BattleField] " ..
          _Lib_Battle_Info.TeamOne.name ..
            " " ..
              Game.getStorageValue(_Lib_Battle_Info.TeamOne.storage) ..
                " VS " .. Game.getStorageValue(_Lib_Battle_Info.TeamTwo.storage) .. " " .. _Lib_Battle_Info.TeamTwo.name
      )
    end
  end
  return true
