...
local function insertItems(buffer, info, parent, items)
  ...
  for _, item in ipairs(items) do
    ...
    pushSeparated(
      ...
      item:getId(),
      item:getSubType(),
      db.escapeBlob(item:serializeAttributes())
    )
    table.insert(buffer, ")")
...
local function insertRewardItems(playerGuid, timestamp, itemList)
  db.asyncStoreQuery(
    "SELECT `pid`, `sid` FROM `player_rewards` WHERE player_id = " .. playerGuid .. " ORDER BY `sid` ASC;",
    function(query)
      local lastReward = 0
      local lastStoreId
      if (query) then
      ...
      local buffer = {
        "INSERT INTO `player_rewards` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `attributes`) VALUES"
      }
...
function onDeath(creature, corpse, killer, mostDamageKiller, lastHitUnjustified, mostDamageUnjustified)
  ...
  if monsterType:isRewardBoss() then -- Make sure it is a boss
    ...
    for guid, stats in pairs(info) do
      ...
      local damageOut, damageIn, healing =
        (stats.damageOut or 0) + (part and part.total or 0),
        stats.damageIn or 0,
        stats.healing or 0
