...
local function getGuildLeaders(guild)
  return filter(
    ...
    function(p)
      return p:getGuildLevel() >= CityWars.minGuildRank
    end
  )
end
    ...
    window:addButton(
      ...
      function(button, choice)
        ...
        if not enemyGuild then
          ...
          if ret == CW_RETURNVALUE_TOOFEWPLAYERS then
            return player:sendCancelMessage(
              "The enemy guild has too few online players to be able to participate in a city war."
            )
          elseif ret == CW_RETURNVALUE_TOOFEWUNIQUEIPS then
            return player:sendCancelMessage(
              "The enemy guild has too few unique ip addresses to be able to participate in a city war."
            )
          end
        end
        ...
        if #enemyLeaders == 0 then
          return player:sendCancelMessage(
            "There is no one online in the enemy guild able to accept a city war request."
          )
        end
        ...
        elseif ret == CW_RETURNVALUE_NOERROR then
          player:sendTextMessage(
            ...
            "The invite was sent. The enemy guild's leader has two minutes to accept it."
          )
            ...
            leader:sendTextMessage(
              ...
              '"' ..
                player:getName() ..
                  '" from guild "' ..
                    guildName ..
                      '" has invited your guild for a city war. If you wish to accept, head to a temple and read the guild book.'
            )
