function onKill(player, target)
  ...
  if (slot.type > 1) then
    return player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Use only 1 weapon or disable experience weapon.") and
      true
  end
  ...
  local expp, ato, level, item =
    MonsterType(target:getName():lower()):getExperience() * Game.getExperienceStage(player:getLevel()),
    getItemAttribute(weapon.uid, "aid"),
    getItemLevel(weapon.uid),
    Item(weapon.uid)
        ...
        if (level == k) then
          if (ato >= p[2]) then
            ...
            player:sendTextMessage(
              ...
              "Congratulations you weapon upgrade from " .. level .. " to level " .. level + 1
            )
            ...
            doPlayerRemoveExperience(
              player,
              -formula_player,
              "You weapon received " .. formula_weapon .. " experience "
            )
