...
function Player:onMoveItem(item, count, fromPosition, toPosition, fromCylinder, toCylinder)
  ...
  if (containerTo) then
    ...
    if (containerTo:getId() == GOLD_POUNCH) then
      if
        (not (item:getId() == ITEM_CRYSTAL_COIN or item:getId() == ITEM_PLATINUM_COIN or item:getId() == ITEM_GOLD_COIN))
       then
  ...
  if self:getStorageValue(lionrock.storages.playerCanDoTasks) - os.time() < 0 then
    ...
    local checkPr = false
    if
      item:getId() == lionrock.items.ruby and toPosition.x == p.ruby.x and toPosition.y == p.ruby.y and
        toPosition.z == p.ruby.z
     then
      -- Ruby
    ...
    if
      item:getId() == lionrock.items.sapphire and toPosition.x == p.sapphire.x and toPosition.y == p.sapphire.y and
        toPosition.z == p.sapphire.z
     then
      -- Sapphire
    ...
    if
      item:getId() == lionrock.items.amethyst and toPosition.x == p.amethyst.x and toPosition.y == p.amethyst.y and
        toPosition.z == p.amethyst.z
     then
      -- Amethyst
      self:sendTextMessage(
        MESSAGE_EVENT_ADVANCE,
    ...
    if
      item:getId() == lionrock.items.topaz and toPosition.x == p.topaz.x and toPosition.y == p.topaz.y and
        toPosition.z == p.topaz.z
     then
      -- Topaz
      self:sendTextMessage(
        MESSAGE_EVENT_ADVANCE,
    ...
    if checkPr == true then
      ...
      if
        lionrock.taskactive.ruby == true and lionrock.taskactive.sapphire == true and
          lionrock.taskactive.amethyst == true and
          lionrock.taskactive.topaz == true
       then
    ...
    if nextEquip and nextEquip:getType():getImbuingSlots() > 0 then
      for i = 1, nextEquip:getType():getImbuingSlots() do
        ...
        if (slotEnchant and type(slotEnchant) == "string") then
          ...
          if
            (typeEnchant ~= "" and typeEnchant ~= "skillShield" and not typeEnchant:find("absorb") and
              typeEnchant ~= "speed")
           then
          ...
          if
            (typeEnchant ~= "hitpointsleech" and typeEnchant ~= "manapointsleech" and typeEnchant ~= "criticaldamage" and
              typeEnchant ~= "skillShield" and
              typeEnchant ~= "magiclevelpoints" and
              not typeEnchant:find("absorb") and
              typeEnchant ~= "speed")
           then
      ...
      if nextEquip and nextEquip:getType():getImbuingSlots() > 0 then
        for i = 1, nextEquip:getType():getImbuingSlots() do
          ...
          if (slotEnchant and type(slotEnchant) == "string") then
            ...
            if
              (typeEnchant == "firedamage" or typeEnchant == "earthdamage" or typeEnchant == "icedamage" or
                typeEnchant == "energydamage" or
                typeEnchant == "deathdamage")
             then
              local weaponType = nextEquip:getType():getWeaponType()
              if weaponType ~= WEAPON_NONE and weaponType ~= WEAPON_SHIELD and weaponType ~= WEAPON_AMMO then
                percentDamage = normalDamage * (enchantPercent / 100)
...
function Player:onMove()
  local haveImbuingBoots =
    self:getSlotItem(CONST_SLOT_FEET) and self:getSlotItem(CONST_SLOT_FEET):getType():getImbuingSlots() or 0
...
function Player:onGainExperience(source, exp, rawExp)
  ...
  if (party) then
    if (party:isSharedExperienceActive() and party:isSharedExperienceEnabled()) then
      ...
      if (totalCount <= 10 and count >= 4) then
        exp = exp * 2
      end
    end
  end
