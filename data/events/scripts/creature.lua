...
function Creature:onTargetCombat(target)
  ...
  if
    ((target:isMonster() and self:isPlayer() and target:getType():isPet() and target:getMaster() == self) or
      (self:isMonster() and target:isPlayer() and self:getType():isPet() and self:getMaster() == target))
   then
...
function Creature:onDrainHealth(
  attacker,
  typePrimary,
  damagePrimary,
  typeSecondary,
      ...
      if item and item:getType():getImbuingSlots() > 0 then
        for i = 1, item:getType():getImbuingSlots() do
          ...
          if (slotEnchant and type(slotEnchant) == "string") then
            ...
            if
              (typeEnchant ~= "" and typeEnchant == "skillShield" or
                typeEnchant:find("absorb") and (reductPrimary or reductSecondary))
             then
