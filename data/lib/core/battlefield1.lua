...
function doBroadCastBattle1(type, msg)
  for _, cid in pairs(Game.getPlayers()) do
    if
      Player(cid):getStorageValue(_Lib_Battle_Info1.TeamOne1.storage1) == 1 or
        Player(cid):getStorageValue(_Lib_Battle_Info1.TeamTwo1.storage1) == 1
     then
...
function getWinnersBattle1(storage1)
  ...
  for _, cid in pairs(Game.getPlayers()) do
    ...
    if player:getStorageValue(storage1) >= 1 then
      if _Lib_Battle_Info1.Reward1.exp1[1] then
        player:addExperience(_Lib_Battle_Info1.Reward1.exp1[2], true)
      end
      if _Lib_Battle_Info1.Reward1.items1[1] then
        player:addItem(_Lib_Battle_Info1.Reward1.items1[2], _Lib_Battle_Info1.Reward1.items1[3])
      end
      if _Lib_Battle_Info1.Reward1.premium_days1[1] then
        player:addPremiumDays(_Lib_Battle_Info1.Reward1.premium_days1[2])
      end
    ...
    "" ..
      c1 ..
        " jogadores" ..
          (c1 > 1 and "s" or "") ..
            " do time " ..
              (Game.getStorageValue(_Lib_Battle_Info1.TeamOne1.storage1) == 0 and _Lib_Battle_Info1.TeamTwo1.name1 or
                _Lib_Battle_Info1.TeamOne1.name1) ..
                " ganharam o evento!"
  resetBattle1()
  OpenWallBattle1()
...
function CheckEvent1(delay1)
  if delay1 > 0 and Game.getStorageValue(_Lib_Battle_Info1.storage_count1) > 0 then
    broadcastMessage(
      "[BattleField] Faltam " ..
        Game.getStorageValue(_Lib_Battle_Info1.storage_count1) .. " jogadores para o evento comecar."
    )
  elseif delay1 == 0 and Game.getStorageValue(_Lib_Battle_Info1.storage_count1) > 0 then
    for _, cid in pairs(Game.getPlayers()) do
      local player = Player(cid)
      if
        player:getStorageValue(_Lib_Battle_Info1.TeamOne1.storage1) == 1 or
          player:getStorageValue(_Lib_Battle_Info1.TeamTwo1.storage1) == 1
       then
        player:teleportTo(player:getTown():getTemplePosition())
