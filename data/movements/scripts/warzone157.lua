if not warzoneConfig then
  warzoneConfig = {
    ...
    [45700] = {
      ...
      teleportTo = Position(33106, 31955, 11), -- Local onde o player ser� teleportado dentro da room
      locked = false,
      storage = 790014, -- storage
      interval = 10 * 60 * 60,
      exit = Position(33001, 31900, 9) -- Exit padr�o
    },
    -- Warzone
    [45702] = {
      -- action do movement
      ...
      teleportTo = Position(33096, 31955, 10), -- Local onde o player ser� teleportado dentro da room
      locked = false,
    ...
    [45701] = {
      ...
      teleportTo = Position(33083, 31904, 12), -- Local onde o player ser� teleportado dentro da room
      locked = false,
...
function onStepIn(creature, item, pos, fromPosition)
  ...
  if creature:getStorageValue(info.storage) > os.time() then
    creature:sendTextMessage(
      ...
      "You have already cleared this warzone in the last ten hours."
    )
    creature:teleportTo(fromPosition)
    return false
  end
