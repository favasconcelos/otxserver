local config = {
  [8009] = {
    storageKey = Storage.ChildrenoftheRevolution.SpyBuilding01,
    text = "An impressive ammount of fish is stored here."
  },
  [8010] = {
    storageKey = Storage.ChildrenoftheRevolution.SpyBuilding02,
    text = "A seemingly endless array of weapon stretches before you into the darkness."
  },
  [8011] = {
    storageKey = Storage.ChildrenoftheRevolution.SpyBuilding03,
...
function onStepIn(creature, item, position, fromPosition)
  ...
  if player:getStorageValue(targetTile.storageKey) < 1 then
    ...
    player:setStorageValue(
      Storage.ChildrenoftheRevolution.Mission02,
      player:getStorageValue(Storage.ChildrenoftheRevolution.Mission02) + 1
    )
